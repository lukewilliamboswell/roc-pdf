...
KernelObjectPlan :: [].{
	...
	## Canonical leaf-object counts from the production-visual resource plan: one object
	...
	Plan :: {
		...
		## The production-visual leaf-deduplicated variant: authored stores are still
...
build_plan : KernelTagged.Plan, KernelColor.Plan, KernelImage.Plan, KernelResourceUse.Plan, KernelContent.Plan, KernelObjectPlan.Limits -> Try(KernelObjectPlan.Plan, KernelObjectPlan.Error)
...
build_text_plan : KernelTagged.Plan, KernelColor.Plan, KernelImage.Plan, KernelResourceUse.TextPlan, KernelContent.Plan, KernelObjectPlan.Limits -> Try(KernelObjectPlan.Plan, KernelObjectPlan.Error)
...
build_canonical_plan : KernelTagged.Plan, KernelColor.Plan, KernelImage.Plan, KernelResourceUse.TextPlan, KernelContent.Plan, KernelObjectPlan.LeafObjectCounts, KernelObjectPlan.Limits -> Try(KernelObjectPlan.Plan, KernelObjectPlan.Error)
...
build_counts : ObjectCounts, KernelObjectPlan.Limits -> Try(KernelObjectPlan.Plan, KernelObjectPlan.Error)
...
page_rows : U64, U64 -> List(KernelObjectPlan.PageObjects)
...
profile_rows : U64, U64 -> List(KernelObjectPlan.ProfileObjects)
...
image_rows : U64, List(Bool) -> List(KernelObjectPlan.ImageObjects)
...
object_id = |number| match KernelObject.ObjectId.from_number(number) {
	Err(_) => {
		crash "checked tagged-visual object number escaped"
	}
	Ok(id) => id
}
...
checked_add : U64, U64 -> Try(U64, KernelObjectPlan.Error)
checked_add = |left, right| match U64.plus_try(left, right) {
	Err(Overflow) => Err(ArithmeticOverflow)
	Ok(value) => Ok(value)
}

checked_times : U64, U64 -> Try(U64, KernelObjectPlan.Error)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "validated tagged-visual object-plan index escaped"
	}
}
