...
KernelGate3TaggedTextStructure :: [].{
	...
	## Post-layout navigation input mirroring the Gate 4 assembler: the
	...
	Plan :: { font_objects : List(KernelPdfFont.Objects), structure : KernelStructure.Plan, work : Work }.{
		...
		## `build_with_facts`.
		build_with_navigation : KernelTagged.Plan, KernelColor.Plan, KernelImage.Plan, KernelContent.Plan, KernelGate3FontObjects.Plan, KernelPdfText.ScenePlan, List(FontInput), KernelMetadata.PlanFacts, NavigationInput, Limits -> Try(Plan, Error)
...
build_plan = |tagged, colors, images, content, font_plan, text, fonts, facts, navigation, limits| {
	...
	prefix = KernelGate2TaggedObjects.Plan.build_with_navigation(tagged, objects, catalog_facts, navigation_facts, limits.object_limits) ? TaggedObjects
	pages = match navigation_plan {
		NoNavigationPlan => KernelGate2PageObjects.Plan.build_with_fonts(prefix, tagged, content, font_plan) ? Pages
		WithNavigationPlan(plan_input) => {
			...
			KernelGate2PageObjects.Plan.build_with_fonts_navigation(prefix, tagged, content, font_plan, $per_page) ? Pages
		}
	}
	...
	navigated = match navigation_plan {
		...
		WithNavigationPlan(plan_input) => {
			...
			var $page_objects = List.with_capacity(KernelGate2Objects.Plan.pages(objects).len())
			var $page_scan = 0
			while $page_scan < KernelGate2Objects.Plan.pages(objects).len() {
				$page_objects = $page_objects.append(list_at(KernelGate2Objects.Plan.pages(objects), $page_scan).page)
			...
			lowered = KernelNavigationObjects.add_objects(
				...
				plan_input.planned,
				{
					...
					structure_elements: KernelGate2Objects.Plan.structure_elements(objects),
				},
