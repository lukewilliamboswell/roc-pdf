...
add_image = |builder, names, resource, objects, planned| match resource.payload {
	Jpeg(jpeg) => {
		added = add_image_stream(builder, names, jpeg.bytes, jpeg.color_space, jpeg.dimensions, NoSoftMaskReference, Dct, UnchangedResource, objects, planned.image)?
	...
	Raster(raster) => {
		...
		image = add_image_stream(builder, names, pixels, raster.color_space, raster.dimensions, soft_mask_reference, Deflate, Generated, objects, planned.image)?
...
add_image_stream : KernelObject.Builder, Names, List(U8), Color.SpaceId, Image.Dimensions, SoftMaskReference, KernelObject.FilterPlan, KernelObject.PayloadKind, KernelGate2Objects.Plan, KernelGate2Objects.StreamObjects -> Try(KernelObject.Builder, KernelGate2ResourceObjects.Error)
add_image_stream = |builder, names, bytes, color_space, dimensions, soft_mask, filter, kind, objects, planned| {
	bits = KernelObject.add_integer(builder, 8) ? Object
	color = KernelObject.add_reference(bits.builder, list_at(KernelGate2Objects.Plan.color_spaces(objects), color_space.index())) ? Object
