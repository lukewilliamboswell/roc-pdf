...
build_plan : KernelPdfText.Plan, List(KernelTextStructure.FontInput), Layout.Size, KernelTextStructure.Limits -> Try(KernelTextStructure.Plan, KernelTextStructure.Error)
build_plan = |text, fonts, page_size, limits| {
	...
	bound = KernelOutputBound.calculate(sealed, xref) ? OutputBound
...
add_names : KernelObject.Builder, U64 -> Try({ builder : KernelObject.Builder, names : Names }, KernelTextStructure.Error)
...
add_pages : KernelObject.Builder, Names -> Try(KernelObject.Builder, KernelTextStructure.Error)
...
add_page : KernelObject.Builder, Names, Layout.Size, U64 -> Try(KernelObject.Builder, KernelTextStructure.Error)
...
add_font_resources : KernelObject.Builder, Names, U64 -> Try({ builder : KernelObject.Builder, value : KernelObject.ValueId }, KernelTextStructure.Error)
...
add_layout : KernelObject.Builder, Layout.Unit -> Try({ builder : KernelObject.Builder, id : KernelObject.ValueId }, KernelTextStructure.Error)
...
object_id = |number| match KernelObject.ObjectId.from_number(number) {
	Err(_) => {
		crash "checked text-layout object number escaped"
	}
	Ok(id) => id
}

checked_add : U64, U64 -> Try(U64, KernelTextStructure.Error)
checked_add = |left, right| match U64.plus_try(left, right) {
	Err(Overflow) => Err(ArithmeticOverflow)
	Ok(value) => Ok(value)
}

checked_times : U64, U64 -> Try(U64, KernelTextStructure.Error)
...
list_at = |items, index| match items.get(index) {
	Err(OutOfBounds) => {
		crash "validated text-layout text-structure index escaped"
	}
	Ok(value) => value
}
