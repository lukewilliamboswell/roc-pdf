...
KernelResourceObjects :: [].{
	...
	## Canonical leaf facts from the production-visual resource plan: for each canonical
	...
	Plan :: { builder : KernelObject.Builder, work : Work }.{
		build : KernelPageObjects.Plan, KernelColor.Plan, KernelImage.Plan, KernelObjectPlan.Plan -> Try(Plan, Error)
		build = |pages, colors, images, objects| build_plan(pages, colors, images, objects)

		## The production-visual leaf-deduplicated variant: one object per canonical
		...
		build_canonical : KernelPageObjects.Plan, KernelColor.Plan, KernelImage.Plan, KernelObjectPlan.Plan, CanonicalLeaves -> Try(Plan, Error)
...
build_plan : KernelPageObjects.Plan, KernelColor.Plan, KernelImage.Plan, KernelObjectPlan.Plan -> Try(KernelResourceObjects.Plan, KernelResourceObjects.Error)
build_plan = |pages, colors, images, objects| {
	added_names = add_names(KernelPageObjects.Plan.builder(pages))?
	profile_store = KernelColor.Plan.store(colors)
	image_store = KernelImage.Plan.store(images)
	profiles = add_profiles(added_names.builder, added_names.names, profile_store.profiles, KernelObjectPlan.Plan.profiles(objects))?
...
build_canonical_leaf_plan : KernelPageObjects.Plan, KernelColor.Plan, KernelImage.Plan, KernelObjectPlan.Plan, KernelResourceObjects.CanonicalLeaves -> Try(KernelResourceObjects.Plan, KernelResourceObjects.Error)
build_canonical_leaf_plan = |pages, colors, images, objects, leaves| {
	added_names = add_names(KernelPageObjects.Plan.builder(pages))?
	profile_store = KernelColor.Plan.store(colors)
	image_store = KernelImage.Plan.store(images)
	profiles = add_canonical_profiles(added_names.builder, added_names.names, profile_store.profiles, leaves.profile_representatives, KernelObjectPlan.Plan.profiles(objects))?
...
add_canonical_profiles : KernelObject.Builder, Names, List(Color.IccProfile), List(U64), List(KernelObjectPlan.ProfileObjects) -> Try({ builder : KernelObject.Builder, bytes : U64 }, KernelResourceObjects.Error)
...
add_canonical_icc_based : KernelObject.Builder, Names, U64, KernelObjectPlan.Plan -> Try({ builder : KernelObject.Builder, id : KernelObject.ValueId }, KernelResourceObjects.Error)
add_canonical_icc_based = |builder, names, ordinal, objects| {
	kind = KernelObject.add_name_value(builder, names.icc_based) ? Object
	reference = KernelObject.add_reference(kind.builder, list_at(KernelObjectPlan.Plan.profiles(objects), ordinal).profile) ? Object
	array = KernelObject.add_array(reference.builder, [kind.id, reference.id]) ? Object
	Ok({ builder: array.builder, id: array.id })
}

add_canonical_images : KernelObject.Builder, Names, List(Image.Resource), KernelResourceObjects.CanonicalLeaves, KernelObjectPlan.Plan -> Try(ImageWork, KernelResourceObjects.Error)
add_canonical_images = |builder, names, images, leaves, objects| {
	planned = KernelObjectPlan.Plan.images(objects)
	var $builder = builder
	var $ordinal = 0
	var $image_rows = 0
	var $raster_bytes = 0
...
add_canonical_image : KernelObject.Builder, Names, Image.Resource, { alpha : List(U8), color : List(U8) }, KernelResourceObjects.CanonicalLeaves, KernelObjectPlan.Plan, KernelObjectPlan.ImageObjects -> Try(ImageWork, KernelResourceObjects.Error)
add_canonical_image = |builder, names, resource, planes, leaves, objects, planned| match resource.payload {
	...
	Raster(raster) => {
		...
		match (raster.alpha, planned.soft_mask) {
			...
			_ => {
				crash "planned production-visual canonical alpha shape escaped"
			}
		}
	}
}

canonical_color_target : KernelResourceObjects.CanonicalLeaves, KernelObjectPlan.Plan, Color.SpaceId -> KernelObject.ObjectId
canonical_color_target = |leaves, objects, space| list_at(KernelObjectPlan.Plan.color_spaces(objects), list_at(leaves.color_names, space.index()))

add_names : KernelObject.Builder -> Try({ builder : KernelObject.Builder, names : Names }, KernelResourceObjects.Error)
add_names = |builder| {
...
add_profiles : KernelObject.Builder, Names, List(Color.IccProfile), List(KernelObjectPlan.ProfileObjects) -> Try({ builder : KernelObject.Builder, bytes : U64 }, KernelResourceObjects.Error)
...
add_cal_gray : KernelObject.Builder, Names, Color.Tristimulus, Color.Tristimulus -> Try({ builder : KernelObject.Builder, id : KernelObject.ValueId }, KernelResourceObjects.Error)
...
add_icc_based : KernelObject.Builder, Names, Color.ProfileId, KernelObjectPlan.Plan -> Try({ builder : KernelObject.Builder, id : KernelObject.ValueId }, KernelResourceObjects.Error)
add_icc_based = |builder, names, profile, objects| {
	kind = KernelObject.add_name_value(builder, names.icc_based) ? Object
	reference = KernelObject.add_reference(kind.builder, list_at(KernelObjectPlan.Plan.profiles(objects), profile.index()).profile) ? Object
	array = KernelObject.add_array(reference.builder, [kind.id, reference.id]) ? Object
	Ok({ builder: array.builder, id: array.id })
}

add_tristimulus : KernelObject.Builder, Color.Tristimulus -> Try({ builder : KernelObject.Builder, id : KernelObject.ValueId }, KernelResourceObjects.Error)
...
add_millionth : KernelObject.Builder, I64 -> Try({ builder : KernelObject.Builder, id : KernelObject.ValueId }, KernelResourceObjects.Error)
add_millionth = |builder, coefficient| {
	decimal = match KernelLex.Decimal.from_coefficient(coefficient, 6) {
		Err(_) => {
			crash "fixed tagged-visual tristimulus scale escaped"
		}
		Ok(valid) => valid
	}
...
add_images : KernelObject.Builder, Names, List(Image.Resource), KernelObjectPlan.Plan -> Try(ImageWork, KernelResourceObjects.Error)
...
add_image = |builder, names, resource, objects, planned| match resource.payload {
	Jpeg(jpeg) => {
		added = add_image_stream(builder, names, jpeg.bytes, list_at(KernelObjectPlan.Plan.color_spaces(objects), jpeg.color_space.index()), jpeg.dimensions, NoSoftMaskReference, Dct, UnchangedResource, planned.image)?
		Ok({ builder: added, image_rows: jpeg.dimensions.height.to_u64(), raster_bytes: jpeg.bytes.len(), soft_mask_rows: 0 })
	}
	Raster(raster) => {
		soft_mask_reference = match planned.soft_mask {
		...
		image = add_image_stream(builder, names, pixels, list_at(KernelObjectPlan.Plan.color_spaces(objects), raster.color_space.index()), raster.dimensions, soft_mask_reference, Deflate, Generated, planned.image)?
		match (raster.alpha, planned.soft_mask) {
			...
			_ => {
				crash "planned tagged-visual alpha shape escaped"
			}
		}
	}
}
...
add_image_stream : KernelObject.Builder, Names, List(U8), KernelObject.ObjectId, Image.Dimensions, SoftMaskReference, KernelObject.FilterPlan, KernelObject.PayloadKind, KernelObjectPlan.StreamObjects -> Try(KernelObject.Builder, KernelResourceObjects.Error)
...
add_soft_mask : KernelObject.Builder, Names, List(U8), Image.Dimensions, KernelObjectPlan.StreamObjects -> Try(KernelObject.Builder, KernelResourceObjects.Error)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "validated tagged-visual resource-object index escaped"
	}
}
...
expect {
	...
	pages = KernelPageObjects.Plan.build(prefix, pipeline.tagged, pipeline.content, pipeline.objects)?
	resources = KernelResourceObjects.Plan.build(pages, pipeline.colors, pipeline.images, pipeline.objects)?
	sealed = KernelSeal.seal(KernelResourceObjects.Plan.builder(resources))?
	counts = KernelSeal.Plan.counts(sealed)
	work = KernelResourceObjects.Plan.work(resources)
...
expect {
	...
	counts = KernelSeal.Plan.counts(sealed)
	work = KernelResourceObjects.Plan.work(resources)
	counts.objects == KernelObjectPlan.Plan.object_count(pipeline.objects) and counts.payloads == 3 and counts.streams == 3 and work.raster_bytes == 8 and work.image_rows == 2 and work.soft_mask_rows == 2
}
