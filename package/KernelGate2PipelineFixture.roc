...
KernelGate2PipelineFixture :: [].{
	...
	pipeline = |_| build_pipeline(False)

	contextual_pipeline : {} -> Try(Pipeline, Error)
	contextual_pipeline = |_| build_pipeline(True)
}
...
build_pipeline : Bool -> Try(KernelGate2PipelineFixture.Pipeline, KernelGate2PipelineFixture.Error)
build_pipeline = |contextual| {
	...
	images = KernelImage.Plan.build(image_sources, colors, KernelImage.Limits.make({ max_decoded_bytes: 4, max_encoded_bytes: 0, max_height: 2, max_markers: 0, max_resources: 1, max_width: 2 })) ? ImageFailure
