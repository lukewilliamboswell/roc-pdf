...
calculate_bound : KernelSeal.Plan, KernelObject.ObjectId -> Try(KernelGate2OutputBound.Bound, KernelGate2OutputBound.Error)
...
value_bound : KernelObject.Store, KernelObject.ValueId -> Try(ValueBound, KernelGate2OutputBound.Error)
...
array_bound : KernelObject.Store, KernelObject.Span -> Try(ValueBound, KernelGate2OutputBound.Error)
...
dictionary_bound : KernelObject.Store, KernelObject.Span -> Try(ValueBound, KernelGate2OutputBound.Error)
...
name_bound : KernelLex.Name -> Try(U64, KernelGate2OutputBound.Error)
...
stream_payload_bound : KernelObject.Store, KernelObject.StreamId -> Try(U64, KernelGate2OutputBound.Error)
...
xref_bound : KernelObject.ObjectId, U64 -> Try(U64, KernelGate2OutputBound.Error)
...
checked_add : U64, U64 -> Try(U64, KernelGate2OutputBound.Error)
checked_add = |left, right| match U64.plus_try(left, right) {
	Err(Overflow) => Err(ArithmeticOverflow)
	Ok(total) => Ok(total)
}

checked_times : U64, U64 -> Try(U64, KernelGate2OutputBound.Error)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "sealed Gate 2 output-bound index escaped"
	}
}
