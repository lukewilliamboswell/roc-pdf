#!/usr/bin/env python3
"""Original-byte checks for the text-layout real UAX #9 right-to-left slice."""
...
SNAPSHOT = ROOT / "tests" / "actual_text" / "rtl_text.pdf"
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-rtl-") as temporary_name:
    ...
    print("PASS text-layout PDFBox extraction matches its pinned right-to-left normalization")


def self_test() -> None:
    original = SNAPSHOT.read_bytes()
    validate_rtl_pdf(original)
    for mutation in (
        ...
        try:
            validate_rtl_pdf(mutation)
        except (ValidationError, ValueError, KeyError):
            continue
        raise SystemExit("text-layout RTL checker accepted an atomic structural negative")
    print("PASS text-layout RTL checker self-test")
...
def main() -> None:
    ...
    validate_rtl_pdf(pdf.read_bytes())
    print(f"PASS text-layout RTL resolved visual order, mirrored presentation, logical ActualText, and ToUnicode checks: {pdf}")
