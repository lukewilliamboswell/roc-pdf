#!/usr/bin/env python3
"""Independent structural checks for the Gate 4 shading and tiling-pattern
fixtures.
...
from check_gate4_forms import check_balance, check_ownership, decoded_stream, replace_once
from check_gate4_transparency import alpha_decimal
...
SHARE_100_SNAPSHOT = ROOT / "tests" / "gate4_shadings_share_100" / "snapshot.pdf"
SHARE_1000_SNAPSHOT = ROOT / "tests" / "gate4_shadings_share_1000" / "snapshot.pdf"
DISTINCT_8_SNAPSHOT = ROOT / "tests" / "gate4_shadings_distinct_8" / "snapshot.pdf"
DISTINCT_64_SNAPSHOT = ROOT / "tests" / "gate4_shadings_distinct_64" / "snapshot.pdf"
STOPS_16_SNAPSHOT = ROOT / "tests" / "gate4_shadings_stops_16" / "snapshot.pdf"
STOPS_64_SNAPSHOT = ROOT / "tests" / "gate4_shadings_stops_64" / "snapshot.pdf"
PSHARE_100_SNAPSHOT = ROOT / "tests" / "gate4_patterns_share_100" / "snapshot.pdf"
PSHARE_1000_SNAPSHOT = ROOT / "tests" / "gate4_patterns_share_1000" / "snapshot.pdf"
PDISTINCT_8_SNAPSHOT = ROOT / "tests" / "gate4_patterns_distinct_8" / "snapshot.pdf"
PDISTINCT_64_SNAPSHOT = ROOT / "tests" / "gate4_patterns_distinct_64" / "snapshot.pdf"
PCELLS_16_SNAPSHOT = ROOT / "tests" / "gate4_patterns_cells_16" / "snapshot.pdf"
PCELLS_64_SNAPSHOT = ROOT / "tests" / "gate4_patterns_cells_64" / "snapshot.pdf"
NEGATIVE_SNAPSHOT = ROOT / "tests" / "gate4_shadings_negative" / "snapshot.pdf"
...
class _FormFactsView:
    ...
    def parent_tree_rows(self, page: int) -> list[int]:
        from check_gate4_forms import FormFacts
...
def validate_gate4_shadings_pdf(pdf: bytes, dimensions: dict[str, int]) -> None:
    if dimensions.get("gate4_shading_showcase"):
        validate_shading_showcase(pdf, dimensions)
    elif dimensions.get("gate4_shading_share") or dimensions.get("gate4_shading_distinct"):
        validate_shading_grid(pdf, dimensions)
    elif dimensions.get("gate4_shading_stops"):
        validate_shading_stops(pdf, dimensions)
    elif dimensions.get("gate4_pattern_share") or dimensions.get("gate4_pattern_distinct") or dimensions.get("gate4_pattern_cells"):
        validate_pattern_grid(pdf, dimensions)
    else:
        raise ValidationError("unknown Gate 4 shading-pattern fixture dimensions")


SHOWCASE_DIMENSIONS = {
    "pages": 1,
    "gate4_shading_showcase": 1,
    "authored_shadings": 7,
...
def self_test() -> None:
    ...
    validate_shading_grid(SHARE_100_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_shading_share": 1, "shading_paints": 100, "canonical_shadings": 1})
    validate_shading_grid(SHARE_1000_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_shading_share": 1, "shading_paints": 1000, "canonical_shadings": 1})
    validate_shading_grid(DISTINCT_8_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_shading_distinct": 1, "canonical_shadings": 8})
    validate_shading_grid(DISTINCT_64_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_shading_distinct": 1, "canonical_shadings": 64})
    validate_shading_stops(STOPS_16_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_shading_stops": 1, "stops": 16, "canonical_functions": 16})
    validate_shading_stops(STOPS_64_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_shading_stops": 1, "stops": 64, "canonical_functions": 64})
    validate_pattern_grid(PSHARE_100_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_pattern_share": 1, "pattern_fills": 100, "canonical_patterns": 1})
    validate_pattern_grid(PSHARE_1000_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_pattern_share": 1, "pattern_fills": 1000, "canonical_patterns": 1})
    validate_pattern_grid(PDISTINCT_8_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_pattern_distinct": 1, "canonical_patterns": 8})
    validate_pattern_grid(PDISTINCT_64_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_pattern_distinct": 1, "canonical_patterns": 64})
    validate_pattern_grid(PCELLS_16_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_pattern_cells": 1, "cell_commands": 16, "canonical_patterns": 1})
    validate_pattern_grid(PCELLS_64_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_pattern_cells": 1, "cell_commands": 64, "canonical_patterns": 1})
    validate_shading_grid(NEGATIVE_SNAPSHOT.read_bytes(), {"pages": 1, "gate4_shading_share": 1, "shading_paints": 1, "canonical_shadings": 1})
    ...
    for label, mutation in mutations:
        try:
            ...
            continue
        raise SystemExit(f"Gate 4 shading-pattern checker accepted mutated {label}")
    ...
    for label, content in (
        ...
        try:
            ...
            continue
        raise SystemExit(f"Gate 4 shading-pattern checker accepted {label} in a pattern stream")
    print("PASS Gate 4 shading-pattern structural checker self-test")
...
def main() -> None:
    ...
    print(f"PASS Gate 4 shading-pattern structural check: {args.pdf}")
