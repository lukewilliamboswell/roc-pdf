#!/usr/bin/env python3
"""Original-byte checks for the Gate 3 advanced `fi` ligature slice."""
...
SNAPSHOT = ROOT / "tests" / "gate3_ligature_text" / "snapshot.pdf"
EXPECTED_TEXT = "fi\n".encode()
EXPECTED_CONTENT = (
    b"/P <</MCID 0>> BDC\n"
    b"q\n"
...
def check_pdfbox_extraction(pdf: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-ligature-") as directory:
    ...
    print("PASS Gate 3 PDFBox extraction preserves logical fi")
...
def self_test() -> None:
    ...
    for mutation in (replace_once(original, b"<0001> <00660069>", b"<0001> <0066006A>"), replace_once(original, b"/Encoding /Identity-H", b"/Encoding /Identity-V")):
        try:
            ...
            continue
        raise SystemExit("Gate 3 ligature checker accepted an atomic structural negative")
    print("PASS Gate 3 ligature checker self-test")
...
def main() -> None:
    ...
    print(f"PASS Gate 3 ligature structure, CID, ActualText, and ToUnicode checks: {pdf}")
