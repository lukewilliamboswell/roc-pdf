#!/usr/bin/env python3
"""Original-byte checks for the text-layout advanced combining-mark slice."""
...
SNAPSHOT = ROOT / "tests" / "actual_text" / "combining_text.pdf"
EXPECTED_TEXT = "A\u0300"
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-combining-") as temporary_name:
    ...
    print("PASS text-layout PDFBox extraction preserves decomposed combining text")
...
def self_test() -> None:
    ...
    for mutation in (
        ...
        try:
            ...
            continue
        raise SystemExit("text-layout combining checker accepted an atomic structural negative")
    print("PASS text-layout combining checker self-test")
...
def main() -> None:
    ...
    for mutation in (
        ...
        try:
            ...
            continue
        raise SystemExit("text-layout combining checker accepted an atomic structural negative")
    print(f"PASS text-layout combining structure, CID, and decomposed ToUnicode checks: {pdf}")
