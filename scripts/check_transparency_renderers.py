#!/usr/bin/env python3
"""Pinned renderer evidence for the production-visual transparency fixtures.
...
from check_form_renderers import compile_java, render_mutool, require

ROOT = Path(__file__).resolve().parents[1]
SHOWCASE_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency.pdf"
SHARE_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency_share_100.pdf"
...
def check_renderers(renderer: Path, working_directory: Path | None, mutool: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-transparency-render-") as temporary_name:
    ...
    print(
        "PASS production-visual transparency renderers: PDFium Chromium 7988 and PDFBox 3.0.8 match the "
...
def self_test() -> None:
    ...
    try:
        ...
        raise SystemExit("production-visual transparency renderer checker accepted a one-channel negative twin")
    ...
    try:
        ...
        raise SystemExit("production-visual transparency renderer checker accepted a pin far from the model")
    print("PASS production-visual transparency renderer checker self-test")
