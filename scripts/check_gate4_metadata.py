#!/usr/bin/env python3
"""Gate 4 metadata, document-language, and output-intent structural checker.
...
MINIMAL_SNAPSHOT = ROOT / "tests" / "gate4_metadata_minimal" / "snapshot.pdf"
UNIQUE_SNAPSHOT = ROOT / "tests" / "gate4_metadata_unique" / "snapshot.pdf"
RETAINED_SNAPSHOT = ROOT / "tests" / "gate4_metadata_retained" / "snapshot.pdf"
SHARE_16_SNAPSHOT = ROOT / "tests" / "gate4_metadata_share_16" / "snapshot.pdf"
SHARE_64_SNAPSHOT = ROOT / "tests" / "gate4_metadata_share_64" / "snapshot.pdf"
TITLE_64_SNAPSHOT = ROOT / "tests" / "gate4_metadata_title_64" / "snapshot.pdf"
TITLE_256_SNAPSHOT = ROOT / "tests" / "gate4_metadata_title_256" / "snapshot.pdf"
NEGATIVE_SNAPSHOT = ROOT / "tests" / "gate4_metadata_negative" / "snapshot.pdf"
FACADE_SNAPSHOT = ROOT / "tests" / "gate4_metadata_facade" / "snapshot.pdf"
FACADE_NEGATIVE_SNAPSHOT = ROOT / "tests" / "gate4_metadata_facade_negative" / "snapshot.pdf"
...
def validate_gate4_metadata_pdf(pdf: bytes, dimensions: dict[str, int]) -> None:
    pages = dimensions.get("pages", 1)
    if dimensions.get("gate4_metadata_minimal", 0) == 1:
        validate_minimal(pdf)
    elif dimensions.get("gate4_metadata_share", 0) == 1:
        validate_share(pdf)
    elif dimensions.get("gate4_metadata_title", 0) == 1:
        validate_title(pdf, dimensions["title_segments"])
    elif dimensions.get("gate4_metadata_facade", 0) == 1:
        validate_facade(pdf, pages)
    elif dimensions.get("gate4_metadata_facade_negative", 0) == 1:
...
def self_test() -> None:
    ...
    print(
        "PASS check_gate4_metadata self-test: canonical XMP packets, /Lang "
