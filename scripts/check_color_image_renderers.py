#!/usr/bin/env python3
"""Pinned renderer evidence for the production-visual color and image leaf fixtures.
...
curve exactly as pinned by the production-visual form renderer checker; its distinct-grid
...
from check_form_renderers import (
    MUTOOL_IMAGE_TONES,
    MUTOOL_VECTOR_TONES,
    compile_java,
    render_mutool,
...
DEDUP_SNAPSHOT = ROOT / "tests" / "color_images" / "color_images_dedup_64.pdf"
DISTINCT_SNAPSHOT = ROOT / "tests" / "color_images" / "color_images_distinct_8.pdf"
...
def check_renderers(renderer: Path, working_directory: Path | None, mutool: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-color-render-") as temporary_name:
    ...
    print(
        "PASS production-visual color-image renderers: PDFium Chromium 7988 and PDFBox 3.0.8 match the "
...
def self_test() -> None:
    ...
    try:
        ...
        raise SystemExit("production-visual color-image renderer checker accepted a one-channel negative twin")
    print("PASS production-visual color-image renderer checker self-test")
