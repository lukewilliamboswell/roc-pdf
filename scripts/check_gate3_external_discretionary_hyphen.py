...
SNAPSHOT = ROOT / "tests" / "gate3_external_discretionary_hyphen" / "snapshot.pdf"
EXPECTED_TEXT = b"ab\n"
EXPECTED_CONTENT = (
    b"/P <</MCID 0>> BDC\n"
    b"q\n"
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-external-hyphen-") as temporary_name:
    ...
    print("PASS Gate 3 PDFBox 3.0.8 extraction: external visible hyphen preserves logical ab")


def self_test() -> None:
    if not SNAPSHOT.is_file() or not SNAPSHOT.read_bytes().startswith(b"%PDF-"):
        print("PASS Gate 3 external discretionary-hyphen checker self-test deferred until its generated fixture exists")
        return
    ...
    print("PASS Gate 3 external discretionary-hyphen structure and extraction checker self-test")
...
def main() -> None:
    ...
    print(f"PASS Gate 3 external discretionary-hyphen structure, CID, presentation, and logical source checks: {pdf}")
