#!/usr/bin/env python3
"""Structural and extraction oracle for the text-layout supplementary-plane slice."""
...
def validate_supplementary_text_pdf(pdf: bytes) -> None:
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-supplementary-") as temporary_name:
    ...
    print("PASS text-layout PDFBox 3.0.8 extraction: exact supplementary-plane UTF-8 source")


def self_test() -> None:
    pdf = SNAPSHOT.read_bytes()
    validate_supplementary_text_pdf(pdf)
    for mutation in (replace_once(pdf, b"<0001> <D83CDD2F>", b"<0001> <D83CDD30>"), replace_once(pdf, b"/Encoding /Identity-H", b"/Encoding /Identity-V")):
        try:
            validate_supplementary_text_pdf(mutation)
        except (ValidationError, ValueError):
            continue
        raise SystemExit("text-layout supplementary checker accepted a structural negative twin")
    print("PASS text-layout supplementary-plane text structure and extraction checker self-test")
...
def main() -> None:
    ...
    validate_supplementary_text_pdf(pdf.read_bytes())
    print(f"PASS text-layout supplementary-plane text structure, CID, and mapping checks: {pdf}")
