#!/usr/bin/env python3
"""Independent structural checks for production-visual Form XObject fixtures.
...
REPEAT_100_SNAPSHOT = ROOT / "tests" / "forms" / "forms_repeat_100.pdf"
REPEAT_1000_SNAPSHOT = ROOT / "tests" / "forms" / "forms_repeat_1000.pdf"
DAG_8_SNAPSHOT = ROOT / "tests" / "forms" / "forms_dag_8.pdf"
DAG_32_SNAPSHOT = ROOT / "tests" / "forms" / "forms_dag_32.pdf"
DEEP_64_SNAPSHOT = ROOT / "tests" / "forms" / "forms_deep_64.pdf"
NEGATIVE_SNAPSHOT = ROOT / "tests" / "forms" / "forms_negative.pdf"
...
def validate_forms_pdf(pdf: bytes, dimensions: dict[str, int]) -> None:
    if dimensions.get("forms_showcase"):
        validate_showcase(pdf, dimensions)
    elif dimensions.get("forms_repeat"):
        validate_repeat(pdf, dimensions)
    elif dimensions.get("forms_dag"):
        validate_dag(pdf, dimensions)
    elif dimensions.get("forms_deep"):
        validate_deep(pdf, dimensions)
    elif dimensions.get("form_text"):
        validate_text(pdf, dimensions)
    else:
        raise ValidationError("unknown production-visual form fixture dimensions")
...
def self_test() -> None:
    ...
    showcase_dimensions = {"pages": 1, "forms_showcase": 1, "authored_forms": 9, "canonical_forms": 5}
    ...
    for label, mutation in mutations:
        try:
            ...
            continue
        raise SystemExit(f"production-visual form checker accepted mutated {label}")
    print("PASS production-visual Form XObject structural checker self-test")
...
def main() -> None:
    ...
    if args.self_test:
        self_test()
        return
    validate_showcase(args.pdf.read_bytes(), {"pages": 1, "forms_showcase": 1, "authored_forms": 9, "canonical_forms": 5})
    print(f"PASS production-visual Form XObject structural check: {args.pdf}")
