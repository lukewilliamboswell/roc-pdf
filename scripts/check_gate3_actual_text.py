...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-actual-text-") as temporary_name:
    ...
    print("PASS Gate 3 PDFBox 3.0.8 extraction: ActualText restores logical fa from visual af")
...
def self_test() -> None:
    ...
    for index, mutation in enumerate(mutations):
        try:
            validate_gate3_actual_text_pdf(mutation)
        except (ValidationError, ValueError):
            continue
        raise SystemExit(f"Gate 3 ActualText checker accepted negative twin {index}")
    content_mutations = (
    ...
    for index, mutation in enumerate(content_mutations):
        try:
            ...
            continue
        raise SystemExit(f"Gate 3 ActualText content checker accepted negative twin {index}")
    print("PASS Gate 3 reordered ActualText structure and extraction checker self-test")
...
def main() -> None:
    ...
    validate_gate3_actual_text_pdf(pdf.read_bytes())
    print(f"PASS Gate 3 reordered ActualText structure, CID, and mapping checks: {pdf}")
