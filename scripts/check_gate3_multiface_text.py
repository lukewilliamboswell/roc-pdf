#!/usr/bin/env python3
"""Structural and extraction oracle for ordered multi-face Gate 3 output."""
...
SNAPSHOT = ROOT / "tests" / "gate3_multiface_text" / "snapshot.pdf"
EXPECTED_TEXT = "C中é\n".encode()
EXPECTED_CONTENT = (
    b"/P <</MCID 0>> BDC\n"
    b"q\n1 0 0 1 72 700 cm\n"
...
def validate_gate3_multiface_text_pdf(pdf: bytes) -> None:
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-multiface-") as temporary_name:
    ...
    print("PASS Gate 3 PDFBox 3.0.8 extraction: exact mixed Latin+CJK source")


def self_test() -> None:
    pdf = SNAPSHOT.read_bytes()
    validate_gate3_multiface_text_pdf(pdf)
    for mutation in (replace_once(pdf, b"/F1_1 ", b"/F1_X "), replace_once(pdf, b"<0001> <4E2D>", b"<0001> <4E2E>")):
        try:
            validate_gate3_multiface_text_pdf(mutation)
        except (ValidationError, ValueError):
            continue
        raise SystemExit("Gate 3 multi-face checker accepted a structural negative twin")
    print("PASS Gate 3 multi-face text structure and extraction checker self-test")
...
def main() -> None:
    ...
    validate_gate3_multiface_text_pdf(pdf.read_bytes())
    print(f"PASS Gate 3 multi-face text structure, two Type 0/CID resources, and mappings: {pdf}")
