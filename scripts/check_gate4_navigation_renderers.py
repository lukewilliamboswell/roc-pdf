#!/usr/bin/env python3
"""Gate 4 navigation renderer and independent-processor evidence.
...
APPEARANCE = ROOT / "tests" / "gate4_navigation_appearance_16" / "snapshot.pdf"
LABELS = ROOT / "tests" / "gate4_navigation_labels_8" / "snapshot.pdf"
...
def run_matrix(renderer: Path | None, working_directory: Path | None, mutool: Path | None) -> None:
    ...
    print(f"PASS Gate 4 navigation renderer evidence: {tail}", flush=True)
...
def self_test() -> None:
    ...
    print(
        "PASS Gate 4 navigation renderer checker self-test: pinned navigation "
