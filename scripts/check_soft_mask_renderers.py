#!/usr/bin/env python3
"""Pinned renderer evidence for the production-visual Form-backed alpha soft-mask
fixtures.
...
REUSE_SNAPSHOT = ROOT / "tests" / "soft_masks" / "soft_masks_reuse_100.pdf"
...
def check_renderers(renderer: Path, working_directory: Path | None, mutool: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-soft-mask-render-") as temporary_name:
    ...
    print(
        "PASS production-visual soft-mask renderers: PDFium Chromium 7988 and PDFBox 3.0.8 match the "
...
def self_test() -> None:
    ...
    try:
        ...
        raise SystemExit("production-visual soft-mask renderer checker accepted a one-channel negative twin")
    ...
    try:
        ...
        raise SystemExit("production-visual soft-mask renderer checker accepted a pin far from the model")
    print("PASS production-visual soft-mask renderer checker self-test")
