#!/usr/bin/env python3
"""Structural and extraction oracle for the Gate 3 supplementary-plane slice."""
...
SNAPSHOT = ROOT / "tests" / "gate3_supplementary_text" / "snapshot.pdf"
EXPECTED_TEXT = "🄯\n".encode()
EXPECTED_CONTENT = (
    b"/P <</MCID 0>> BDC\n"
    b"q\n"
...
def validate_gate3_supplementary_text_pdf(pdf: bytes) -> None:
    ...
    require(b"/MarkInfo << /Marked true >>" in bodies[catalog], "supplementary catalog does not declare marked content")
    ...
    require(b"/Subtype /Type0" in type0_body and b"/Encoding /Identity-H" in type0_body, "supplementary page font is not Identity-H Type 0")
    descendants = dictionary_ref_array(type0_body, b"DescendantFonts")
    require(len(descendants) == 1, "supplementary Type 0 font must have one descendant")
    cid_body = bodies[descendants[0]]
    require(b"/Subtype /CIDFontType2" in cid_body and b"/W [0 [656 914]]" in cid_body, "supplementary CID widths are not the inspected Inter metrics")
    ...
    require(b"/Length1 5732" in font_dictionary, "supplementary sanitized FontFile2 length differs")
    require(font_bytes.startswith(b"\x00\x01\x00\x00"), "supplementary FontFile2 is not a TrueType-flavoured sfnt")
    require(hashlib.sha256(font_bytes).hexdigest() == EXPECTED_SUBSET_SHA256, "supplementary sanitized subset digest differs")


def check_pdfbox_extraction(pdf: Path) -> None:
    require(PDFBOX_JAR.is_file(), f"vendored PDFBox JAR does not exist: {PDFBOX_JAR}")
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-supplementary-") as temporary_name:
    ...
    print("PASS Gate 3 PDFBox 3.0.8 extraction: exact supplementary-plane UTF-8 source")


def self_test() -> None:
    pdf = SNAPSHOT.read_bytes()
    validate_gate3_supplementary_text_pdf(pdf)
    for mutation in (replace_once(pdf, b"<0001> <D83CDD2F>", b"<0001> <D83CDD30>"), replace_once(pdf, b"/Encoding /Identity-H", b"/Encoding /Identity-V")):
        try:
            validate_gate3_supplementary_text_pdf(mutation)
        except (ValidationError, ValueError):
            continue
        raise SystemExit("Gate 3 supplementary checker accepted a structural negative twin")
    print("PASS Gate 3 supplementary-plane text structure and extraction checker self-test")
...
def main() -> None:
    ...
    validate_gate3_supplementary_text_pdf(pdf.read_bytes())
    print(f"PASS Gate 3 supplementary-plane text structure, CID, and mapping checks: {pdf}")
