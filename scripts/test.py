...
from check_gate4_navigation import validate_gate4_navigation_pdf
...
def run_case(
    ...
    if expected_pages is not None:
        ...
        elif any(key.startswith("gate4_soft_mask") for key in case.dimensions):
            validate_gate4_soft_masks_pdf(result.stdout, case.dimensions)
            print(f"PASS {case.name}: canonical Alpha mask states, direct /SMask /G wiring to isolated mask forms outside every resource dictionary, balanced mask groups, and placement-site ownership facts", flush=True)
        ...
        elif any(key.startswith("gate4_navigation") for key in case.dimensions):
            validate_gate4_navigation_pdf(result.stdout, case.dimensions)
            print(
                f"PASS {case.name}: paired /SD + /D actions, keyboard-ordered /Annots, OBJR/ParentTree linkage, named destinations, outline preorder, page labels, and appearance geometry verified structurally",
                flush=True,
            )
...
def main() -> None:
    ...
    command(sys.executable, "scripts/check_gate4_navigation.py", "--self-test")
    command(sys.executable, "scripts/check_gate4_navigation_renderers.py", "--self-test")
    if not args.update_snapshots:
