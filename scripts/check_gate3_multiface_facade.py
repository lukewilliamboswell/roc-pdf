#!/usr/bin/env python3
"""Structural oracle for the Gate 3 public ordered multi-face facade slice."""
...
SNAPSHOT = ROOT / "tests" / "gate3_multiface_facade" / "snapshot.pdf"
EXPECTED_TEXT = "C中é\n".encode()
EXPECTED_CONTENT = (
    b"/P <</MCID 0>> BDC\n"
    b"q\n"
...
def validate_gate3_multiface_facade_pdf(pdf: bytes) -> None:
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-multiface-facade-") as temporary_name:
    ...
    print("PASS Gate 3 PDFBox extraction preserves the mixed-face source text")


def self_test() -> None:
    original = SNAPSHOT.read_bytes()
    validate_gate3_multiface_facade_pdf(original)
    for mutation in (
        ...
        try:
            validate_gate3_multiface_facade_pdf(mutation)
        except (ValidationError, ValueError):
            continue
        raise SystemExit("Gate 3 multiface facade checker accepted an atomic structural negative")
    print("PASS Gate 3 multiface facade checker self-test")
...
def main() -> None:
    ...
    validate_gate3_multiface_facade_pdf(pdf.read_bytes())
    print(f"PASS Gate 3 multiface facade structure, dense font resources, CID, and per-font ToUnicode checks: {pdf}")
