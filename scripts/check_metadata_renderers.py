#!/usr/bin/env python3
"""Pinned renderer evidence for the production-visual metadata/output-intent fixtures.
...
production-visual slices pinned. Rendering does not and cannot validate metadata or
output-intent correctness — that is the structural checker's and the
validators' job (`check_metadata.py`; veraPDF is recorded separately as
diagnostic evidence).
...
SHARE_SNAPSHOT = ROOT / "tests" / "metadata" / "metadata_share_64.pdf"
...
def run_matrix(renderer: Path, working_directory: Path | None, mutool: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="metadata-renderers-") as name:
        temporary = Path(name)
    ...
    print(
        "PASS production-visual metadata renderers: PDFium Chromium 7988 and PDFBox 3.0.8 render the "
...
def self_test() -> None:
    ...
    print("PASS production-visual metadata renderer checker self-test")
