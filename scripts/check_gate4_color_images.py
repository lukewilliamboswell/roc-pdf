#!/usr/bin/env python3
"""Independent structural checks for Gate 4 color and image leaf fixtures.
...
- deduplicated authored twins resolve to one shared object from page and form
  ...
  (reusing the Gate 4 form checker's used-equals-declared rule).
"""
...
DEDUP_8_SNAPSHOT = ROOT / "tests" / "gate4_color_images_dedup_8" / "snapshot.pdf"
DEDUP_64_SNAPSHOT = ROOT / "tests" / "gate4_color_images_dedup_64" / "snapshot.pdf"
DISTINCT_8_SNAPSHOT = ROOT / "tests" / "gate4_color_images_distinct_8" / "snapshot.pdf"
DISTINCT_64_SNAPSHOT = ROOT / "tests" / "gate4_color_images_distinct_64" / "snapshot.pdf"
NEGATIVE_SNAPSHOT = ROOT / "tests" / "gate4_color_images_negative" / "snapshot.pdf"
...
def validate_gate4_color_images_pdf(pdf: bytes, dimensions: dict[str, int]) -> None:
    if dimensions.get("gate4_color_image_showcase"):
        validate_color_image_showcase(pdf, dimensions)
    elif dimensions.get("gate4_color_image_distinct") or dimensions.get("gate4_color_image_dedup"):
        validate_color_image_grid(pdf, dimensions)
    else:
        raise ValidationError("unknown Gate 4 color-image fixture dimensions")


SHOWCASE_DIMENSIONS = {
    "pages": 1,
    "gate4_color_image_showcase": 1,
    "authored_profiles": 2,
...
def self_test() -> None:
    ...
    validate_color_image_grid(
        ...
        {"pages": 1, "gate4_color_image_dedup": 1, "image_placements": 8, "canonical_images": 1},
    )
    validate_color_image_grid(
        DEDUP_64_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_color_image_dedup": 1, "image_placements": 64, "canonical_images": 1},
    )
    validate_color_image_grid(
        DISTINCT_8_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_color_image_distinct": 1, "image_placements": 8, "canonical_images": 8},
    )
    validate_color_image_grid(
        DISTINCT_64_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_color_image_distinct": 1, "image_placements": 64, "canonical_images": 64},
    )
    validate_color_image_grid(
        NEGATIVE_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_color_image_dedup": 1, "image_placements": 1, "canonical_images": 1},
    )
    ...
    for label, mutation in mutations:
        try:
            ...
            continue
        raise SystemExit(f"Gate 4 color-image checker accepted mutated {label}")
    print("PASS Gate 4 color-image structural checker self-test")
...
def main() -> None:
    ...
    print(f"PASS Gate 4 color-image structural check: {args.pdf}")
