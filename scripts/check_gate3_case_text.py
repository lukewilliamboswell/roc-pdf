#!/usr/bin/env python3
"""Original-byte checks for the Gate 3 case-transformation slice."""
...
SNAPSHOT = ROOT / "tests" / "gate3_case_text" / "snapshot.pdf"
...
def validate_gate3_case_pdf(pdf: bytes) -> None:
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-case-") as temporary_name:
    ...
    print("PASS Gate 3 PDFBox extraction recovers the authored source through ActualText")


def self_test() -> None:
    original = SNAPSHOT.read_bytes()
    validate_gate3_case_pdf(original)
    for mutation in (
        ...
        try:
            validate_gate3_case_pdf(mutation)
        except (ValidationError, ValueError, KeyError):
            continue
        raise SystemExit("Gate 3 case checker accepted an atomic structural negative")
    print("PASS Gate 3 case transformation checker self-test")
...
def main() -> None:
    ...
    validate_gate3_case_pdf(pdf.read_bytes())
    print(f"PASS Gate 3 case transformation presentation, logical ActualText, CID, and ToUnicode checks: {pdf}")
