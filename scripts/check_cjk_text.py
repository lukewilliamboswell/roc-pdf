#!/usr/bin/env python3
"""Structural and extraction oracle for the text-layout one-scalar CJK slice."""
...
def validate_cjk_text_pdf(pdf: bytes) -> None:
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-cjk-") as temporary_name:
    ...
    print("PASS text-layout PDFBox 3.0.8 extraction: exact CJK UTF-8 source")


def self_test() -> None:
    pdf = SNAPSHOT.read_bytes()
    validate_cjk_text_pdf(pdf)
    for mutation in (replace_once(pdf, b"<0001> <4E2D>", b"<0001> <4E2E>"), replace_once(pdf, b"/Encoding /Identity-H", b"/Encoding /Identity-V")):
        try:
            validate_cjk_text_pdf(mutation)
        except (ValidationError, ValueError):
            continue
        raise SystemExit("text-layout CJK checker accepted a structural negative twin")
    print("PASS text-layout CJK text structure and extraction checker self-test")
...
def main() -> None:
    ...
    validate_cjk_text_pdf(pdf.read_bytes())
    print(f"PASS text-layout CJK text structure, CID, and mapping checks: {pdf}")
