...
SNAPSHOT = ROOT / "tests" / "gate3_caller_text" / "snapshot.pdf"
...
def self_test() -> None:
    ...
    validate_gate3_caller_text_pdf(pdf)
    mutations = (
    ...
    for index, mutation in enumerate(mutations):
        try:
            validate_gate3_caller_text_pdf(mutation)
        except ValidationError:
            continue
        raise SystemExit(f"Gate 3 caller text checker accepted negative twin {index}")
    print("PASS Gate 3 caller-font text structure, identity, and mapping checker self-test")
...
def main() -> None:
    ...
    validate_gate3_caller_text_pdf(pdf.read_bytes())
    print(f"PASS Gate 3 caller-font structure, identity, CID, and ToUnicode checks: {pdf}")
