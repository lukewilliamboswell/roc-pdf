#!/usr/bin/env python3
"""Pinned renderer evidence for the production-visual shading and tiling-pattern
fixtures.
...
SHARE_SNAPSHOT = ROOT / "tests" / "shading_patterns" / "shadings_share_100.pdf"
PSHARE_SNAPSHOT = ROOT / "tests" / "shading_patterns" / "patterns_share_100.pdf"
...
def check_renderers(renderer: Path, working_directory: Path | None, mutool: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-shading-render-") as temporary_name:
    ...
    print(
        "PASS production-visual shading-pattern renderers: PDFium Chromium 7988 and PDFBox 3.0.8 match the "
...
def self_test() -> None:
    ...
    try:
        ...
        raise SystemExit("production-visual shading renderer checker accepted a one-channel negative twin")
    print("PASS production-visual shading-pattern renderer checker self-test")
