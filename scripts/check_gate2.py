...
def validate_gate2_pdf(pdf: bytes) -> None:
    ...
    require(len(namespace_refs) == 1, "Gate 2 requires exactly one PDF 2.0 namespace")
...
def self_test() -> None:
    ...
    for index, mutation in enumerate(mutations):
        try:
            validate_gate2_pdf(mutation)
        except ValidationError:
            continue
        raise SystemExit(f"Gate 2 checker accepted negative twin {index}")
    print("PASS Gate 2 normalized structure checker self-test")
...
def main() -> None:
    ...
    if args.self_test:
        self_test()
        return
    validate_gate2_pdf(args.pdf.read_bytes())
    print(f"PASS Gate 2 normalized structure check: {args.pdf}")
