#!/usr/bin/env python3
"""Original-byte checks for the Gate 3 generated-list-label slice."""
...
from check_gate3_renderers import InkMetrics, assert_geometry_agreement, compile_pdfbox_renderer, ink_metrics
from check_gate3_text import PDFBOX_JAR, PDFBOX_SOURCE
from check_gate2_renderers import read_ppm
from check_pdf_structure import ValidationError, object_slices, require


ROOT = Path(__file__).resolve().parents[1]
SELF_TEST_SNAPSHOT = ROOT / "tests" / "gate3_generated_label" / "snapshot.pdf"
...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-generated-label-") as temporary_name:
    ...
    print("PASS Gate 3 generated labels PDFBox 3.0.8 extraction: exact logical labels and bodies")
...
def check_renderers(renderer: Path, working_directory: Path | None, pdf: Path) -> None:
    ...
    require(PDFBOX_JAR.is_file(), f"vendored PDFBox JAR does not exist: {PDFBOX_JAR}")
    with tempfile.TemporaryDirectory(prefix="roc-pdf-gate3-generated-label-render-") as temporary_name:
    ...
    print("PASS Gate 3 generated-label renderers: independently pinned exact 72-dpi PDFBox and PDFium metrics")
...
def self_test() -> None:
    ...
    try:
        ...
        raise SystemExit("Gate 3 generated-label checker accepted an atomic bullet-mapping negative")
    print("PASS Gate 3 generated-label structural checker self-test")
...
def main() -> None:
    ...
    print(f"PASS Gate 3 generated-label structure, typed list ownership, CID, and ToUnicode checks: {pdf}")
