#!/usr/bin/env python3
"""Independent structural checks for Gate 4 canonical font-leaf fixtures.
...
SHOWCASE_SNAPSHOT = ROOT / "tests" / "gate4_font_leaves" / "snapshot.pdf"
FACTS_SNAPSHOT = ROOT / "tests" / "gate4_font_leaf_facts" / "snapshot.pdf"
SHARE_100_SNAPSHOT = ROOT / "tests" / "gate4_font_leaves_share_100" / "snapshot.pdf"
DEDUPE_8_SNAPSHOT = ROOT / "tests" / "gate4_font_leaves_dedupe_8" / "snapshot.pdf"
DISTINCT_8_SNAPSHOT = ROOT / "tests" / "gate4_font_leaves_distinct_8" / "snapshot.pdf"
...
def validate_gate4_fonts_pdf(pdf: bytes, dimensions: dict[str, int]) -> None:
    if dimensions.get("gate4_font_showcase"):
        validate_showcase(pdf, dimensions)
    elif dimensions.get("gate4_font_facts"):
        validate_facts(pdf, dimensions)
    elif dimensions.get("gate4_font_share"):
        validate_scaled(pdf, dimensions, 1, dimensions["gate4_font_share"])
    elif dimensions.get("gate4_font_dedupe"):
        validate_scaled(pdf, dimensions, 1, dimensions["gate4_font_dedupe"])
    elif dimensions.get("gate4_font_distinct"):
        validate_scaled(pdf, dimensions, dimensions["gate4_font_distinct"], dimensions["gate4_font_distinct"])
    else:
        ## The unique/retained ownership fixtures reuse the showcase
        ## document, and the collision/negative carriers reuse the one-run
        ## sharing document.
        if dimensions.get("gate4_font_unique") or dimensions.get("gate4_font_retained"):
            validate_showcase(pdf, dimensions)
        else:
...
def self_test() -> None:
    ...
    print("PASS check_gate4_fonts self-test: canonical bundles, subset verification, and mutation rejections")
