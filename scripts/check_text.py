...
def check_pdfbox_extraction(pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-pdf-extract-") as temporary_name:
    ...
    print("PASS text-layout PDFBox 3.0.8 extraction: exact UTF-8 Café PDF")
...
def self_test() -> None:
    ...
    for index, mutation in enumerate(mutations):
        try:
            validate_text_pdf(mutation)
        except ValidationError:
            continue
        raise SystemExit(f"text-layout text checker accepted negative twin {index}")
    print("PASS text-layout text structure and mapping checker self-test")
...
def main() -> None:
    ...
    validate_text_pdf(pdf.read_bytes())
    print(f"PASS text-layout visible text structure, font, CID, and ToUnicode checks: {pdf}")
