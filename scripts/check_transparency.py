#!/usr/bin/env python3
"""Independent structural checks for production-visual transparency fixtures.
...
SHOWCASE_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency.pdf"
SHARE_100_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency_share_100.pdf"
SHARE_1000_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency_share_1000.pdf"
STATES_100_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency_states_100.pdf"
STATES_1000_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency_states_1000.pdf"
NEST_16_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency_nest_16.pdf"
NEST_64_SNAPSHOT = ROOT / "tests" / "transparency" / "transparency_nest_64.pdf"
...
def validate_transparency_pdf(pdf: bytes, dimensions: dict[str, int]) -> None:
    if dimensions.get("transparency_showcase"):
        validate_transparency_showcase(pdf, dimensions)
    elif dimensions.get("transparency_share") or dimensions.get("transparency_states"):
        validate_transparency_grid(pdf, dimensions)
    elif dimensions.get("transparency_nest"):
        validate_transparency_nest(pdf, dimensions)
    elif dimensions.get("transparency_forms"):
        validate_transparency_forms(pdf, dimensions)
    else:
        raise ValidationError("unknown production-visual transparency fixture dimensions")


SHOWCASE_DIMENSIONS = {
    "pages": 1,
    "transparency_showcase": 1,
    "opacity_commands": 10,
    "opacity_groups": 9,
...
def self_test() -> None:
    ...
    validate_transparency_grid(
        ...
        {"pages": 1, "transparency_share": 1, "opacity_groups": 100, "canonical_ext_g_states": 1},
    )
    validate_transparency_grid(
        SHARE_1000_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_share": 1, "opacity_groups": 1000, "canonical_ext_g_states": 1},
    )
    validate_transparency_grid(
        STATES_100_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_states": 1, "opacity_groups": 100, "canonical_ext_g_states": 100},
    )
    validate_transparency_grid(
        STATES_1000_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_states": 1, "opacity_groups": 1000, "canonical_ext_g_states": 1000},
    )
    validate_transparency_nest(
        NEST_16_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_nest": 1, "nest_depth": 16},
    )
    validate_transparency_nest(
        NEST_64_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_nest": 1, "nest_depth": 64},
    )
    validate_transparency_forms(
        FORMS_8_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_forms": 1, "opacity_forms": 8},
    )
    validate_transparency_forms(
        FORMS_32_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_forms": 1, "opacity_forms": 32},
    )
    validate_transparency_grid(
        NEGATIVE_SNAPSHOT.read_bytes(),
        {"pages": 1, "transparency_share": 1, "opacity_groups": 1, "canonical_ext_g_states": 1},
    )
    ...
    for label, mutation in mutations:
        try:
            ...
            continue
        raise SystemExit(f"production-visual transparency checker accepted mutated {label}")
    print("PASS production-visual transparency structural checker self-test")
...
def main() -> None:
    ...
    print(f"PASS production-visual transparency structural check: {args.pdf}")
