#!/usr/bin/env python3
"""Independent structural checks for Gate 4 Form-backed alpha soft-mask
fixtures.
...
from check_gate4_forms import check_ownership, replace_once
from check_gate4_transparency import (
    FORM_GROUP,
    TransparencyFacts,
    alpha_decimal,
)
...
REUSE_100_SNAPSHOT = ROOT / "tests" / "gate4_soft_masks_reuse_100" / "snapshot.pdf"
REUSE_1000_SNAPSHOT = ROOT / "tests" / "gate4_soft_masks_reuse_1000" / "snapshot.pdf"
MASKS_8_SNAPSHOT = ROOT / "tests" / "gate4_soft_masks_masks_8" / "snapshot.pdf"
MASKS_64_SNAPSHOT = ROOT / "tests" / "gate4_soft_masks_masks_64" / "snapshot.pdf"
CHAIN_2_SNAPSHOT = ROOT / "tests" / "gate4_soft_masks_chain_2" / "snapshot.pdf"
CHAIN_4_SNAPSHOT = ROOT / "tests" / "gate4_soft_masks_chain_4" / "snapshot.pdf"
NEGATIVE_SNAPSHOT = ROOT / "tests" / "gate4_soft_masks_negative" / "snapshot.pdf"
...
def validate_gate4_soft_masks_pdf(pdf: bytes, dimensions: dict[str, int]) -> None:
    if dimensions.get("gate4_soft_mask_showcase"):
        validate_soft_mask_showcase(pdf, dimensions)
    elif dimensions.get("gate4_soft_mask_reuse") or dimensions.get("gate4_soft_mask_distinct"):
        validate_soft_mask_grid(pdf, dimensions)
    elif dimensions.get("gate4_soft_mask_chain"):
        validate_soft_mask_chain(pdf, dimensions)
    else:
        raise ValidationError("unknown Gate 4 soft-mask fixture dimensions")


SHOWCASE_DIMENSIONS = {
    "pages": 1,
    "gate4_soft_mask_showcase": 1,
...
def self_test() -> None:
    ...
    validate_soft_mask_grid(
        ...
        {"pages": 1, "gate4_soft_mask_reuse": 1, "mask_groups": 100, "canonical_mask_states": 1},
    )
    validate_soft_mask_grid(
        REUSE_1000_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_soft_mask_reuse": 1, "mask_groups": 1000, "canonical_mask_states": 1},
    )
    validate_soft_mask_grid(
        MASKS_8_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_soft_mask_distinct": 1, "mask_groups": 8, "canonical_mask_states": 8},
    )
    validate_soft_mask_grid(
        MASKS_64_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_soft_mask_distinct": 1, "mask_groups": 64, "canonical_mask_states": 64},
    )
    validate_soft_mask_chain(
        CHAIN_2_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_soft_mask_chain": 1, "mask_chain": 2},
    )
    validate_soft_mask_chain(
        CHAIN_4_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_soft_mask_chain": 1, "mask_chain": 4},
    )
    validate_soft_mask_grid(
        NEGATIVE_SNAPSHOT.read_bytes(),
        {"pages": 1, "gate4_soft_mask_reuse": 1, "mask_groups": 1, "canonical_mask_states": 1},
    )
    ...
    for label, mutation in mutations:
        try:
            ...
            continue
        raise SystemExit(f"Gate 4 soft-mask checker accepted mutated {label}")
    print("PASS Gate 4 soft-mask structural checker self-test")
...
def main() -> None:
    ...
    print(f"PASS Gate 4 soft-mask structural check: {args.pdf}")
