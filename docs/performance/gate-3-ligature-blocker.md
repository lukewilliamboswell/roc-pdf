# Gate 3 ligature-output dependency audit

## Chosen slice and evidence boundary

The next smallest Gate 3 text-matrix slice would be one end-to-end Latin
...
an `fi` ligature mapping. The existing `Gate3ShapeEvidence` value called
...
dev-backend allocation evidence. This record does not close Gate 3 or broaden
