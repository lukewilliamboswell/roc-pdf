...
| Public and caller-font performance/retention evidence | Satisfied for the current one-face facade | The shared-registry and deliberately unique-registry controls measure two complete public outputs, exact input-copy/retention/inspection facts, and original-byte structural output inspection (`gate-3-caller-font-retention.md`). Facade list and compact-builder scale fixtures remain registered. | Ordered multi-face public selection is still a separate local implementation gap and must add its own ownership/cache evidence when it becomes the facade path. |
...
and positive discretionary-hyphen output. The caller-font policy and remaining
cache evidence are also incomplete. The current
