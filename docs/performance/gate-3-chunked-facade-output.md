# Gate 3 authored chunked facade output
...
over it. The former Gate 1 blank-only request validator is deleted; no caller
remains.
...
existing `tests/gate3_pdf_facade/snapshot.pdf` (12,397 bytes, SHA-256
...
`tests/gate3_pdf_facade_chunks/main.roc` authors the exact
`gate3_pdf_facade` document and collects the chunk stream. Two spec cases
...
- Both modes measure 2,845 dev allocations. The modes are identical here
  ...
  diverge for the caller-resource chunk sharing of Gate 4 resource
  documents, which authored Gate 3 output does not carry.
...
`tests/gate3_pdf_facade_chunks_negative/main.roc` proves the atomic
