# text-layout bounded bidirectional prerequisite
...
fixture emits a blank structural-kernel PDF only to use the common scenario protocol; the
...
Until those facts exist, all non-single-level inputs remain rejected and text-layout
remains open.
...
recorded in [`rtl-text.md`](rtl-text.md).
