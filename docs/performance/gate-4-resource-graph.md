# Gate 4 resource dependency graph and deterministic resource identity

This records the design, complexity, ownership, and evidence for the first
Gate 4 slice: the resource-planning foundation. It covers the direct-edge
...
**Scope.** This is the reusable resource-planning boundary that later Gate 4
...
- `package/KernelResourceGraph.roc` — resource identity, descriptors, payload
  ...
  deduplication.
- `package/Gate4ResourceGraphEvidence.roc` — scaled scenarios, adversarial
...
store, and `KernelResourceUse` counts Gate 2 colour and image use inside one
scene. Neither carries dependency edges, payload identity, descriptor facts, or
placement ownership, so neither can express the Gate 4 contract. They are left
untouched; Gate 4 owns a separate boundary. This matches `architecture.md`
...
`gate4-resource-graph-v1`, measured on the pinned dev backend at
`before_fixture_main`):
