# production-visual Form XObjects

This records the design, ownership model, complexity, and evidence for the
second production-visual slice: Form XObjects as an end-to-end PDF 2.0 capability built on
the resource-planning foundation in
[resource-graph.md](resource-graph.md). It does not close production-visual.
...
- `KernelFormObjects.roc` — deterministic object planning: form stream
  objects (and font objects when text is present) appended after the tagged-visual
  base plan.
- `KernelPageObjects.roc` — gains `Plan.build_with_page_resources`, the
  per-page-resource-dictionary variant of the existing page lowering.
- `KernelFormStructure.roc` — assembles tagged, page, resource, form, and
  font objects into one sealed deterministic plan.
- `FormEvidence.roc`, `FormTextEvidence.roc` — scenario evidence.
- `scripts/check_forms.py` — the independent structural checker.
- `scripts/check_form_renderers.py` — pinned PDFium/PDFBox rendering and
...
3. MCIDs, page `/StructParents`, and ParentTree rows therefore remain exactly
   the page-stream facts the tagged-visual tagged plan already produces. No form
...
- text commands serialized as the run's prepared `ActualText`/body bytes from
  ...
  ([font-leaves.md](font-leaves.md)) later replaced this with
...
color-image slice ([color-image-leaves.md](color-image-leaves.md))
...
([font-leaves.md](font-leaves.md)) derived canonical font
...
- Object planning appends, after the unchanged tagged-visual base plan: one
...
- Transparency `/Group`, soft masks, isolated/knockout groups, blend modes,
  ...
  ([transparency.md](transparency.md)) later made constant
...
- Font leaf deduplicated object emission (landed in
  [font-leaves.md](font-leaves.md)) and annotation
...
`form-xobjects-v1`, measured on the pinned dev backend at
...
Independent structural checking is `scripts/check_forms.py`; pinned
rendering, extraction, and the version-scoped nested-form depth limits of the
pinned readers are `scripts/check_form_renderers.py`. The showcase,
...
zero channel tolerance; the text-in-form fixture reproduces the text-layout text
...
production-visual/5 capability. That run is recorded as tool validation, not as any
conformance claim; the XMP/output-intent capability has since landed in
[metadata-output-intent.md](metadata-output-intent.md), which
absorbed the 6.7.2.1 finding for metadata-bearing documents.

The compiler-defect workaround in `FormTextEvidence` (all const-evaluable
