# production-visual canonical metadata, document language, and sRGB output intent
...
optional typed input on the canonical production-visual kernel path, where the intent's
...
XMP metadata stream) that every earlier production-visual slice recorded as deferred.
...
- Integration through all three lowering paths: the authored facade pipeline,
  the facade's blank-document structural path, and the canonical production-visual
  kernel path (`KernelFormStructure.build_with_facts`).

Explicitly not in this slice:

- PDF/A identification metadata (`pdfaid:part`/`pdfaid:rev`): that schema is
  the PDF/A-4 conformance claim itself and belongs to static PDF/A-4 capability. veraPDF's
...
- Namespace declarations and properties follow the contract-definition
...
`/DestOutputProfile` stream is the ordinary profile stream shape every production-visual
...
- `KernelTaggedObjects` adds the fact-only names conditionally and
...
- On the facade path (`KernelFacadePipeline` → `KernelFacadeOutput` →
  `KernelTaggedTextStructure.build_with_facts`), the scene stage's
...
- Empty facade documents route through
  ...
  the sealed-store digest (`KernelIdentity`), so distinct facts produce
  distinct file identifiers where the fact-free blank kernel path keeps its
  constant `Blank` identity.
- On the canonical production-visual path
  (`KernelFormStructure.build_with_facts`), the intent's authored
...
language). `MetadataEvidence` expects run the whole canonical pipeline
under `./scripts/test.py capability fixture tests`, including the adversarial showcase,
sharing checks, the xref invariant, and the 18-step negative sweep.

Harness cases (`tests/spec.json`, revision
`metadata-output-intent-v1`, exact x64musl allocations under the
...
`scripts/check_metadata.py` parses the emitted bytes directly and, for
...
`scripts/check_metadata_renderers.py` runs the pinned
...
earlier production-visual slice deferred is resolved by this slice. The identification
schema is deliberately not emitted here because it *is* the PDF/A-4
conformance claim, which belongs to static PDF/A-4 capability's validated whitelist; emitting
...
- Bytes: each facade snapshot grows by its packet plus the 3,024-byte
  profile plus the intent/catalog syntax (for example `pdf_facade`
...
- `blank` (a facade fixture despite its name) now carries the facts
  ...
  derivation stays covered by the kernel-path `pages` stress case.
  The structural-kernel kernel evidence itself (Evidence fixtures and
...
whitelist enforcement remain static PDF/A-4 capability; PDF/UA-2 metadata requirements
(`dc:title` agreement with `DisplayDocTitle`, `/MarkInfo`, `/Tabs`) remain
accessible-vocabulary capability — the canonical `dc:title` this slice emits is the prerequisite they
build on.
