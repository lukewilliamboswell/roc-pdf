# structural-kernel catalog and page-leaf lowering slice
...
`balanced-indexes.md`; the outline hierarchy is recorded in
`outline-hierarchy.md`. Arlington object-model validation is recorded in
`arlington.md`; warning-free non-lenient PDFBox parsing and exact object
facts are recorded in `pdfbox.md`. Unchanged-resource retention evidence
is recorded separately in `resource-retention.md`.
