# production-visual canonical font leaves

This records the design, semantics, ownership model, complexity, and evidence
for the seventh production-visual slice: canonical resource identity for sanitized
embedded font subsets and deduplicated font-leaf emission — the text-layout Type 0
font pipeline integrated end to end with the production-visual direct-edge resource
...
graph ([resource-graph.md](resource-graph.md)), the Form
XObject machinery ([form-xobjects.md](form-xobjects.md)), the
canonical leaves ([color-image-leaves.md](color-image-leaves.md)),
and the text-layout font slices
([font-planning.md](font-planning.md),
[font-subsetting.md](font-subsetting.md),
[pdf-font-objects.md](pdf-font-objects.md)). It does not close
production-visual and claims no PDF/A-4 or PDF/UA-2 conformance.
...
- No shaping, coverage, or Unicode capability change; text-layout boundaries are
...
objects the text-layout lowering emits per font (FontFile2 stream and its length,
...
- Validated source font faces, shaping instances, and glyph-closure plans
  ...
  nodes. The registry-level sharing story for source faces is the text-layout
  contract ([caller-font-retention.md](caller-font-retention.md))
  and is unchanged.
...
The deterministic subset tag is the unchanged text-layout policy: six uppercase
...
- **Font selection is naming, so it moved to lowering.** Prepared run
  ...
  (authored ordinal on the tagged-visual/3 paths — byte-identical output — and
  the canonical map on the production-visual path). This removes the form slice's
...
- **Objects.** `KernelFormObjects` plans `9 × canonical fonts`
  objects after the patterns; `KernelFormStructure` emits one bundle
...
- The inspected source allocation itself remains owned by the text-layout
  ...
  [caller-font-retention.md](caller-font-retention.md) and
...
`FontLeafEvidence.roc`. Harness cases (`tests/spec.json`,
scenario revision `font-leaves-v1`, measured on the pinned dev
...
production-visual checkers.

`scripts/check_font_renderers.py` runs the pinned matrix live:
...
- **production-visual canonical-path fixtures, −1 allocation per planner run.**
...
- **text-layout scene-text fixtures, +6 to +72 allocations.** The `Tf`
...
- **`production-visual text inside a meaningful form`, bytes unchanged.** Both causes
...
structural-kernel/2 fixtures and every fontless byte snapshot are untouched.

## Exact remaining production-visual work

- XMP metadata, document language, and output intents — landed in
  [metadata-output-intent.md](metadata-output-intent.md),
