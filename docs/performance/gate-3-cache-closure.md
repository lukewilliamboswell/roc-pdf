# Gate 3 bounded-cache closure record

This record aggregates the closure-level evidence for every cache kind the
roadmap declares for Gate 3 — font parsing/coverage, shaping, measurement,
...
retained result and never reparses. `gate-3-caller-font-retention.md` and
the two-face `shared-registry` mode of
`gate-3-multiface-public-facade.md` prove one retained payload and one
...
`Gate3FontEvidence.multi_face_probe_scale` (x1000/x10000) measures
...
`Gate3FacadeEvidence.ordered_facade` shows the facade performs those visits
...
(the `gate3-facade-batch-shaping-v1` scale rows registered in
`tests/spec.json` and reviewed in `gate-3-facade-shaping.md`). The ordered
...
instance, size, source, and width. The `gate3-facade-cached-line-layout-v1`
scale rows registered in `tests/spec.json` and reviewed in
`gate-3-facade-line-layout.md` prove hit/miss behaviour for the single-face
...
the bounded probe table (the `gate3-facade-source-cache-v1` shared and
unique scale rows registered in `tests/spec.json` and reviewed in
`gate-3-facade-source-cache.md`, with `adjacent_hits` for the common
repeated-input fast path). This is the identity every cache above keys on.

## Hyphenation — inapplicable

Automatic hyphenation is not claimed at Gate 3: no language pattern set is
accepted, `gate-3-soft-hyphen.md` records that only explicit caller-selected
