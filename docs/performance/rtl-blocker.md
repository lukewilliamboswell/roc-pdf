# text-layout real RTL output dependency audit

## Chosen slice and evidence boundary

The next missing text-layout matrix row is one real right-to-left paragraph with a
...
review are recorded in `bidi-boundary.md`.
...
separate and leaves text-layout open.
...
recorded in [`rtl-text.md`](rtl-text.md). The bounded
single-level prerequisite described above has been replaced by that real
resolver; `bidi-boundary.md` remains the historical record of the
