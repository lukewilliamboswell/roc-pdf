# production-visual color and image leaves

This records the design, ownership model, complexity, and evidence for the
third production-visual slice: the production color and image resource pipeline —
...
graph in [resource-graph.md](resource-graph.md) and the form
machinery in [form-xobjects.md](form-xobjects.md). It does not
...
- `KernelContent.roc` — content naming through the canonical maps: `/CS` and
  ...
  to (the tagged-visual/3 paths keep authored naming unchanged).
- `KernelObjectPlan.roc` — `Plan.build_canonical`: object identities planned
  for canonical leaf counts while authored stores are still cross-checked.
- `KernelResourceObjects.roc` — `Plan.build_canonical`: one object per
...
- `KernelFormStructure.roc` — assembles the canonical leaf objects and
...
- `ColorImageEvidence.roc` — scenario evidence.
- `scripts/check_color_images.py` — the independent structural checker.
- `scripts/check_color_image_renderers.py` — pinned PDFium/PDFBox/MuPDF
  rendering evidence.
...
- **Font** — at this slice, still the caller-supplied payload, 1:1 with
  ...
  ([font-leaves.md](font-leaves.md)) later derived canonical
...
permuted in the multi-form fixtures. The production-visual form snapshots and their
...
- Output intents and 6.7.2.1 metadata — landed in
  [metadata-output-intent.md](metadata-output-intent.md):
  canonical XMP, the catalog language, and the packaged sRGB output intent
  over this slice's canonical profile machinery. (The transparency page
  `/Group` this slice originally deferred under veraPDF's 6.2.9 rule landed
  with the transparency slice,
  [transparency.md](transparency.md), which regenerated this
...
- Canonical font leaf identity and font-leaf deduplication (landed in
  [font-leaves.md](font-leaves.md)).
- CMYK, Separation/DeviceN/spot color, overprint, luminosity masks,
  non-Normal blending, shadings, and patterns (later production-visual slices or
...
`color-image-leaves-v1`, measured on the pinned dev backend at
...
Independent structural checking is `scripts/check_color_images.py`: it
...
Pinned rendering is `scripts/check_color_image_renderers.py`. The
