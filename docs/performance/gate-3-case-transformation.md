# Gate 3 case-transformation output

This closes the case-transformation row of the Gate 3 extraction matrix: one
...
`roc-lang/unicode` 4.0.0 (`gate-3-unicode-4.0.0-adoption.md`) supplies `Case`,
...
for this row; `scripts/check_gate3_case_text.py` asserts both halves and
...
per engine in `check_gate3_renderers.py` (`--case`). The embedded sanitized
