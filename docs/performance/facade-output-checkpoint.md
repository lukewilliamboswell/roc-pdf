# text-layout facade output checkpoint
...
This is a work-in-progress checkpoint. It does not close text-layout or claim that
...
[public-pdf-facade.md](public-pdf-facade.md).
...
```sh
roc build tests/facade_output/app.roc --opt=dev --output=facade-output-probe
```
...
2. Audit every text-layout roadmap row, including caller-font behavior and
   performance bounds, before adding the text-layout closure record.
