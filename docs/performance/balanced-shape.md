# structural-kernel shared balanced shape

## Representation rule

`KernelBalanced.Shape` is the shared topology for structural-kernel balanced structures.
...
`balanced-indexes.md`. The structurally distinct linked outline hierarchy
is recorded in `outline-hierarchy.md`.
