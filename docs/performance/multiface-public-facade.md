# text-layout public ordered multi-face facade output
...
`tests/multiface_facade/multiface_facade.roc` registers the 7,816-byte caller
...
`scripts/check_multiface_facade.py` verifies these facts directly with
mutation self-tests; PDFBox 3.0.8 extracts exactly `C中é\n`; PDFBox/PDFium
72-dpi ink metrics are pinned per engine in `check_text_renderers.py`
(`--multiface-facade`).

## Negatives

`tests/multiface_facade/multiface_facade_negative.roc` proves four stable typed
...
`font` evidence fixture and are deliberately not duplicated here: the
facade consumes the same `Font.Registry.plan` boundary those cases already
prove.

## Retention

Extending `caller-font-retention.md` to two faces:
...
Registry level (`FontEvidence.multi_face_probe_scale`, x1000/x10000):
...
Facade level (`FacadeEvidence.ordered_facade`, x1000/x10000): N
...
evidence recorded in [`cache-closure.md`](cache-closure.md).
