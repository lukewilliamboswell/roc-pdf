# Gate 4 navigation: links, destinations, outlines, page labels, and annotation appearances

This records the design, semantics, ownership model, complexity, and
evidence for the final Gate 4 capability slice: URI and internal GoTo link
...
end to end on both the canonical Gate 4 kernel path and the public `Pdf`
facade. It is built on the resource graph
([gate-4-resource-graph.md](gate-4-resource-graph.md)), the Form XObject
machinery ([gate-4-form-xobjects.md](gate-4-form-xobjects.md)), the
canonical leaves and font bundles
([gate-4-color-image-leaves.md](gate-4-color-image-leaves.md),
[gate-4-font-leaves.md](gate-4-font-leaves.md)), the metadata slice
([gate-4-metadata-output-intent.md](gate-4-metadata-output-intent.md)), and
the Gate 1 outline and balanced-index planners
([gate-1-outline-hierarchy.md](gate-1-outline-hierarchy.md),
[gate-1-balanced-indexes.md](gate-1-balanced-indexes.md)). It does not
close Gate 4 — the separate closure audit remains — and claims no PDF/A-4
...
- **Outlines**: authored dense preorder with explicit open state, sealed by
  the unchanged Gate 1 `KernelOutline` planner and lowered as the linked
...
`build_text_navigation` accept the annotation store the Gate 0 types always
...
`KernelGate2OutputBound` enforces `xref == objects + 1`. The tagged prefix
...
extended `/ParentTreeNextKey`. The facade's Gate 3 structure builder
mirrors the Gate 4 assembler exactly.
...
`examples/pdf_facade.roc`. `Gate4NavigationEvidence` runs the whole
canonical pipeline under `roc test package/evidence.roc`, including the
adversarial showcase byte-compare and the 32-step negative sweep.

Harness cases (`tests/spec.json`, revision
`gate4-navigation-annotations-v1`, exact x64musl allocations under the
...
`scripts/check_gate4_navigation.py` parses the emitted bytes directly and
...
`scripts/check_gate4_navigation_renderers.py` runs the pinned matrix live.
...
6.7.3-1 (PDF/A identification schema, Gate 5); on the metadata-less kernel
...
flag set, which is precisely a profile-eligibility rule for Gate 5's
...
- **Every tagged plan allocates its dense annotation-owner and
  ...
  documents), which every kernel fixture from Gate 2 up shows as +1 to +6
  depending on how many plans the scenario builds.
- **Page dictionaries assemble their entry lists by conditional append**
  (the `/Annots`/`/Group` refactor in KernelGate2PageObjects; a constant
...
The Gate 4 closure audit remains, as its own follow-up. Every roadmap
capability line of Gate 4 is now implemented with its evidence: resource
