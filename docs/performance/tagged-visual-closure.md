# tagged-visual closure review

## Outcome

The minimal tagged visual kernel capability is complete. This is a private compiler-
pipeline capability boundary: it does not make the future text-layout authoring
...
| tagged-visual boundary | Implementation and focused evidence |
| --- | --- |
| Checked page boxes, rotation, fixed-point coordinates, transforms, paths, clipping, paint state, and bounded graphics nesting | `KernelGeometry`, `KernelScene`, `geometry.md` |
| PDF 2.0 `Document`/`P` semantics, namespace, contextual Artifact, occurrence reverse index, paint ownership, MCIDs, ParentTree, and mixed `/K` | `KernelSemantics`, `KernelTagged`, `KernelTaggedObjects`, `semantics.md`, `tagged-plan.md` |
| Typed gray/RGB color, bounded raster/JPEG inspection and sanitization, alpha, and resource reuse | `KernelColor`, `KernelImage`, `KernelResourceUse`, `KernelResourceObjects`, `resources.md` |
| Canonical content, stable resource names, sealed object identities, checked emission bound, file identity, and deterministic PDF bytes | `KernelContent`, `KernelResourceName`, `KernelObjectPlan`, `KernelOutputBound`, `KernelTaggedStructure`, `content.md`, `objects.md` |
| Exact normalized emitted structure and independent negative twins | `scripts/check_tagged_visual.py`, `negative.md` |
| Independent appearance and hand-constructed pixel oracle | PDFium Chromium 7988, PDFBox 3.0.8, `scripts/check_visual_renderers.py`, `renderers.md` |
...
oracles. text-layout may now build searchable international text and the useful
