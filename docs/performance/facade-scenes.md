# text-layout facade page scenes and text ownership
...
text-layout currently accepts only exact sRGB black from facade styles and prepares
...
ICCBased sRGB and broader color support belong to production-visual.
...
`tests/facade_scenes` uses matched prepared-input controls around the
flat arena boundary:
