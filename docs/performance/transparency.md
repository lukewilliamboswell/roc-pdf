# production-visual transparency: constant opacity, Normal blending, and isolated groups

This records the design, semantics, ownership model, complexity, and evidence
for the fourth production-visual slice: PDF 2.0 transparency as an end-to-end capability
built on the resource graph
([resource-graph.md](resource-graph.md)), the Form XObject
machinery ([form-xobjects.md](form-xobjects.md)), and the
canonical color/image/ICC leaves
([color-image-leaves.md](color-image-leaves.md)). It makes the
existing typed `Scene.Opacity` capability executable for the supported subset.
It does not close production-visual and claims no PDF/A-4 or PDF/UA-2 conformance.
...
([soft-masks.md](soft-masks.md)). Including them here would have forced the
...
- `KernelScene.roc` — form-aware scenes validate opacity groups like any
  nested command range (`Resources.with_forms` enables the capability; tagged-visual/3
...
- `KernelFormObjects.roc` — one dictionary object per canonical state,
  planned between form streams and font objects (`build_with_states`; a zero
  state count leaves every existing plan identical).
- `KernelPageObjects.roc` — `build_with_page_groups`, the per-page
  transparency `/Group` entry.
- `KernelFormStructure.roc` — ExtGState objects, `/ExtGState` dictionary
...
- `TransparencyEvidence.roc` — scenario evidence.
- `scripts/check_transparency.py` — the independent structural checker.
- `scripts/check_transparency_renderers.py` — the pinned
...
revision `transparency-v1`, measured on the pinned dev backend at
...
The twelve atomic negatives: opacity in a tagged-visual scene; non-opaque
...
`scripts/check_transparency.py` parses the emitted bytes directly and
...
`scripts/check_transparency_renderers.py` renders the original bytes
...
blending space). All other structural-kernel–4 snapshots are byte-identical, kept so
...
- URI links, typed internal destinations with paired `/SD` + `/D`, named
  ...
  in [navigation-annotations.md](navigation-annotations.md).
...
the accessible-vocabulary capability vocabulary, so no closure claim is made here.
