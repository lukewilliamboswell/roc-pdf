# text-layout caller-font facade composition
...
The focused public fixture `tests/caller_font/caller_facade.roc` registers the
...
The registered fixture is now `text-layout caller-font facade output` in
...
`text-layout caller-font facade restricted-rights atomic negative` supplies
...
This is public caller-font composition evidence, not text-layout closure. The
...
or broader multilingual caller-font output remains an explicit unmet text-layout
dependency.

The local ordered-policy planning seam is now recorded in
[`multiface-font-selection.md`](multiface-font-selection.md).
...
[`caller-font-retention.md`](caller-font-retention.md). It closes
...
[`cache-closure.md`](cache-closure.md).
