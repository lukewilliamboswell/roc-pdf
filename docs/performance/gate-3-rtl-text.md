# Gate 3 resolved right-to-left output

This closes the real UAX #9 right-to-left row of the Gate 3
...
`roc-lang/unicode` 4.0.0 (`gate-3-unicode-4.0.0-adoption.md`) supplies
...
`Gate3BidiEvidence.uax9_vectors` transcribes seven expectations from the
...
`scripts/check_gate3_rtl.py` verifies all of this on the original bytes with
...
per engine in `check_gate3_renderers.py` (`--rtl`).
