# production-visual linear/radial shadings and colored tiling patterns

This records the design, semantics, ownership model, complexity, and
evidence for the sixth production-visual slice: PDF 2.0 axial/radial shadings and a
...
and the transparency slices ([transparency.md](transparency.md),
[soft-masks.md](soft-masks.md)). It does not close production-visual and
...
Every production-visual pipeline fixture now derives the (empty) paint layout during
...
and structural-kernel–3 fixtures kept their exact allocation counts. The full pinned
...
`KernelContent`) and in `ShadingPatternEvidence.roc`. Harness cases
(`tests/spec.json`, scenario revision `shadings-patterns-v1`,
...
The 27 atomic negatives: the tagged-visual and production-visual form-constructor checks;
...
`scripts/check_shadings.py` parses the emitted bytes directly and
...
`scripts/check_shading_renderers.py` renders the original bytes with
