...
## production-visual text-inside-a-form evidence.
##
...
## `ToUnicode`, and `ActualText` behavior is produced by the unchanged text-layout
## text lowering.
##
...
Fixture :: [].{
	...
	text_form = |runtime_guard| {
		...
		structure_work = KernelFormStructure.Plan.work(sample.structure)
		Ok({
			bytes: sample.bytes,
			work: [
...
build_sample = |prelude, scene_store| {
	...
	objects = KernelFormObjects.Plan.build_with_states(base, KernelForm.Plan.canonical_form_count(form_plan), KernelForm.Plan.canonical_state_count(form_plan), KernelForm.Plan.canonical_font_count(form_plan), 32) ? |_| FormObjectFailure
...
check_text_negatives : Prelude -> Try(U64, Fixture.EvidenceError)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "production-visual form text evidence index escaped"
	}
}
