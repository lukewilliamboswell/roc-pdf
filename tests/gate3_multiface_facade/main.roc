...
main! = |args| {
	...
	match mode {
		...
		_ => crash "Gate 3 multiface facade mode is invalid"
	}
}
...
positive = |runtime_argument_count| {
	if runtime_argument_count != 2 {
		crash "Gate 3 multiface facade argument count is invalid"
	}
...
shared_registry = |runtime_argument_count| {
	if runtime_argument_count != 2 {
		crash "Gate 3 multiface facade shared-registry argument count is invalid"
	}
	registered = register_faces(runtime_argument_count)
	theme = Theme.with_font_policy(Theme.default, registered.policy)
...
unique_registries = |runtime_argument_count| {
	if runtime_argument_count != 2 {
		crash "Gate 3 multiface facade unique-registries argument count is invalid"
	}
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => crash "Gate 3 multiface facade argument missing"
}
