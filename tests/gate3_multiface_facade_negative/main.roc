...
main! = |args| {
	...
	match mode {
		...
		_ => crash "Gate 3 multiface facade negative mode is invalid"
	}
}
...
rejected = |runtime_argument_count, text, expected| {
	if runtime_argument_count != 2 {
		crash "Gate 3 multiface facade negative argument count is invalid"
	}
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => crash "Gate 3 multiface facade negative argument missing"
}
