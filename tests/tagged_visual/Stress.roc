...
run_stress : U64, U8 -> Try(Stress.Report, Stress.Error)
	...
	if command_count == 0 {
		...
		if phase == 1 {
			...
			with_content = if phase == 2 {
				...
				semantic_store = KernelFixture.semantics
...
stress_scene = |command_count| {
	placement = KernelFixture.rect(0, 0, 1000, 1000)
	commands = List.repeat(DrawImage({ image: Image.Id.from_index(0), placement }), command_count)
	page_box = KernelFixture.rect(0, 0, 10000, 10000)
	{
		commands,
		dash_lengths: [],
...
checked_add : U64, U64 -> Try(U64, Stress.Error)
checked_add = |left, right| match U64.plus_try(left, right) {
	Err(Overflow) => Err(ArithmeticOverflow)
	Ok(total) => Ok(total)
}

checked_times : U64, U64 -> Try(U64, Stress.Error)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "validated tagged-visual stress index escaped"
	}
}
