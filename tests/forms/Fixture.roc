...
import pdf.KernelForm
import pdf.KernelObjectPlan
import pdf.KernelFormObjects
import pdf.KernelFormStructure
import pdf.KernelImage
import pdf.KernelObject
import pdf.KernelPdfFont
import pdf.KernelResourceGraph
...
## production-visual Form XObject evidence.
##
...
leaf_objects : KernelForm.Plan -> KernelObjectPlan.LeafObjectCounts
leaf_objects = |plan| {
...
Built := {
	...
	structure : KernelFormStructure.Plan,
...
structure_limits : KernelFormStructure.Limits
structure_limits = KernelFormStructure.Limits.make({
...
BuildFailure := [
	...
	ObjectFailure(KernelObjectPlan.Error),
	FormObjectFailure(KernelFormObjects.Error),
	EmitFailure,
	ResourceUseFailure(KernelResourceUse.Error),
	SemanticFailure(KernelSemantics.Error),
	StructureFailure(KernelFormStructure.Error),
...
run_pipeline = |input| {
	...
	base = KernelObjectPlan.Plan.build_canonical(tagged, stores.colors, stores.images, resource_use, content, leaf_objects(form_plan), KernelObjectPlan.Limits.make({ max_objects: 8192, max_pages: 1 })) ? ObjectFailure
	objects = KernelFormObjects.Plan.build_with_states(base, KernelForm.Plan.canonical_form_count(form_plan), KernelForm.Plan.canonical_state_count(form_plan), 0, 8192) ? FormObjectFailure
	structure = KernelFormStructure.Plan.build(tagged, stores.colors, stores.images, content, form_plan, objects, NoTextObjects, structure_limits) ? StructureFailure
	bytes = KernelEmit.to_bytes(KernelFormStructure.Plan.structure(structure)) ? |_| EmitFailure
...
work_vector = |built| {
	...
	structure_work = KernelFormStructure.Plan.work(built.structure)
	[
...
check_showcase_sharing : Built -> Try({}, Fixture.EvidenceError)
...
expect_scene_rejection : U64, Scenario, (KernelScene.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
expect_form_limit_rejection : U64, Scenario, KernelScene.FormLimits, (KernelScene.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
expect_facts_rejection : U64, Scenario, (KernelForm.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
expect_tagged_rejection : U64, Scenario, (KernelTagged.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
build_objects : Scenario, U64 -> Try(KernelFormObjects.Plan, BuildFailure)
build_objects = |input, max_objects| {
	...
	resource_use = KernelResourceUse.TextPlan.build_with_forms(form_scene, stores.colors, stores.images) ? ResourceUseFailure
	base = KernelObjectPlan.Plan.build_canonical(tagged, stores.colors, stores.images, resource_use, content, leaf_objects(form_plan), KernelObjectPlan.Limits.make({ max_objects: 8192, max_pages: 1 })) ? ObjectFailure
	plan = KernelFormObjects.Plan.build_with_states(base, KernelForm.Plan.canonical_form_count(form_plan), KernelForm.Plan.canonical_state_count(form_plan), 0, max_objects) ? FormObjectFailure
	Ok(plan)
}
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "production-visual form evidence index escaped"
	}
}
...
list_set = |items, index, value| match items.set(index, value) {
	Ok(next) => next
	Err(OutOfBounds) => {
		crash "production-visual form evidence update escaped"
	}
}
