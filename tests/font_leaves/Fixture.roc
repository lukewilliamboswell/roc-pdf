...
run_pipeline = |spec, guard| {
	...
	base = KernelObjectPlan.Plan.build_canonical(
		...
		KernelObjectPlan.Limits.make({ max_objects: 8192, max_pages: 4 }),
	) ? ObjectFailure
	objects = KernelFormObjects.Plan.build_with_states(
		base,
	...
	structure = KernelFormStructure.Plan.build(
		tagged,
		colors,
		images,
		content,
...
run_collision : U64 -> Try({ bytes : List(U8), work : List(U64) }, Fixture.EvidenceError)
run_collision = |scale| {
...
check_sharing : Built, U64, U64, U64 -> Try({}, Fixture.EvidenceError)
...
build_negative_context : U64 -> Try(NegativeContext, Fixture.EvidenceError)
...
expect_leaf_rejection : U64, KernelFontLeaf.Bundle, (KernelFontLeaf.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
check_negatives = |guard| {
	...
	mismatch_result = KernelFormStructure.Plan.build(
		pipeline.tagged,
		pipeline.colors,
		pipeline.images,
		full.content,
...
complete_pipeline : PipelinePrefix, KernelFontLeaf.Leaf -> Try({ content : KernelContent.Plan, form_plan : KernelForm.Plan, objects : KernelFormObjects.Plan }, BuildFailure)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "production-visual font-leaf evidence index escaped"
	}
}
...
list_set = |items, index, value| match items.set(index, value) {
	Ok(next) => next
	Err(OutOfBounds) => {
		crash "production-visual font-leaf evidence update escaped"
	}
}
