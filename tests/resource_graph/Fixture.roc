...
## production-visual resource-planning evidence.
##
...
blank_pdf : {} -> Try(List(U8), Fixture.EvidenceError)
blank_pdf = |{}| {
...
build_scenario : Str, U64 -> Try(KernelResourceGraph.Plan, Fixture.EvidenceError)
...
build_reversed : Str, U64 -> Try(KernelResourceGraph.Plan, Fixture.EvidenceError)
...
build_collision : U64 -> Try(KernelResourceGraph.Plan, Fixture.EvidenceError)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "production-visual resource-graph evidence index escaped"
	}
}
...
list_set = |items, index, value| match items.set(index, value) {
	Ok(next) => next
	Err(OutOfBounds) => {
		crash "production-visual resource-graph evidence update escaped"
	}
}
