...
import pdf.KernelDiscretionaryHyphen
import pdf.KernelBidiBoundary
import pdf.KernelCaseTransform
...
import "../assets/IBMPlexSansHebrew-Rtl-Fixture.ttf" as rtl_font_bytes : List(U8)
...
Fixture :: [].{
	...
	ligature_text = |runtime_guard| {
		...
		Ok({
			...
			work: [
				...
				KernelTaggedTextStructure.Plan.work(sample.structure).objects,
				bytes.len(),
			],
		})
	}
	...
	multi_face_text = |runtime_guard| {
		if runtime_guard != 0 {
			return Err(InvalidRuntimeGuard)
		}
		sample = build_multi_face_sample({}) ? |_| EvidenceFailure
		bytes = KernelEmit.to_bytes(KernelTaggedTextStructure.Plan.structure(sample.structure)) ? |_| EvidenceFailure
		text_work = KernelPdfText.ScenePlan.work(sample.text)
		structure_work = KernelTaggedTextStructure.Plan.work(sample.structure)
		Ok({
			bytes,
			work: [
				sample.selection.work.grapheme_visits,
				sample.selection.work.face_visits,
				sample.selection.work.coverage_span_visits,
				sample.selection.face_ranges.len(),
				sample.latin.bytes.len(),
				sample.cjk.bytes.len(),
				sample.shape.work.run_visits,
				sample.shape.work.cluster_visits,
				sample.shape.work.glyph_visits,
				sample.latin_plan.entries.len(),
				sample.cjk_plan.entries.len(),
				text_work.mappings,
	...
	multi_face_negative : U64 -> Try({ bytes : List(U8), work : List(U64) }, [EvidenceFailure, InvalidRuntimeGuard])
	multi_face_negative = |runtime_guard| {
		if runtime_guard != 0 {
			return Err(InvalidRuntimeGuard)
		}
		registry = multi_face_registry({}) ? |_| EvidenceFailure
		rejected = match registry.plan({
			clusters: [{ scalars: [0x43], script: Font.Script.from_iso15924("Hani"), source: multi_face_range(0) }],
		...
		## This harness carrier is not generated by the rejected request.
		blank = KernelStructure.build_blank(1, A4) ? |_| EvidenceFailure
		bytes = KernelEmit.to_bytes(blank) ? |_| EvidenceFailure
		Ok({ bytes, work: [1, 0, bytes.len()] })
	}
...
MultiFaceSample := {
	cjk : KernelFont.Inspection,
	cjk_plan : KernelFontPlan.Plan,
	latin : KernelFont.Inspection,
	latin_plan : KernelFontPlan.Plan,
	selection : Font.Plan,
...
## and mirroring are pinned independently in `Fixture`.
rtl_source : Str
...
rtl_store = |resolved| {
	...
	while $visual_index < visual.len() {
		...
		glyph = match KernelFont.glyph_for_scalar(resolved.font, scalar) {
			Some(value) => value
			None => crash "text-layout RTL fixture font lacks a required glyph"
		}
		width = match KernelFont.advance_width(resolved.font, glyph) {
			Ok(value) => value
			Err(_) => crash "text-layout RTL fixture font lacks a required advance"
		}
...
scaled_advance = |width, units_per_em| {
	if units_per_em == 0 {
		crash "text-layout RTL fixture font has no em square"
	}
...
build_rtl_sample_after_shape = |font, semantic, shape| {
	...
	base_objects = KernelObjectPlan.Plan.build_with_text(tagged, colors, images, resource_use, content, KernelObjectPlan.Limits.make({ max_objects: 32, max_pages: 1 })) ? |_| ObjectFailure
	font_objects = KernelFontObjects.Plan.build(base_objects, 1, 32) ? |_| FontObjectFailure
...
required_case_glyph = |font, scalar| match KernelFont.glyph_for_scalar(font, scalar) {
	Some(glyph) => glyph
	None => crash "text-layout case fixture font lacks a required glyph"
}
...
case_advance = |font, glyph| {
	width = match KernelFont.advance_width(font, glyph) {
		Ok(value) => value
		Err(_) => crash "text-layout case fixture font lacks a required advance"
	}
	units = font.metrics.units_per_em.to_u64()
	if units == 0 {
		crash "text-layout case fixture font has no em square"
	}
...
build_multi_face_sample = |_| {
	...
	selection = match registry.plan({
		clusters: [
			{ scalars: [0x43], script: Font.Script.from_iso15924("Latn"), source: multi_face_range(0) },
			{ scalars: [0x4e2d], script: Font.Script.from_iso15924("Hani"), source: multi_face_range(1) },
			{ scalars: [0x00e9], script: Font.Script.from_iso15924("Latn"), source: multi_face_range(2) },
		],
		language: Language("en-AU"),
		policy: Font.PolicyId.from_index(2),
		source: Semantics.TextSourceId.from_index(0),
	...
	if selection.face_ranges.len() != 3 {
		return Err(EvidenceFailure)
	}
	latin = registry.prepared_face(Font.FaceId.from_index(0)) ? |_| EvidenceFailure
	cjk = registry.prepared_face(Font.FaceId.from_index(1)) ? |_| EvidenceFailure
	store = multi_face_store(
		selection.face_ranges,
		required_glyph(latin, 0x43) ? |_| EvidenceFailure,
		required_glyph(cjk, 0x4e2d) ? |_| EvidenceFailure,
		required_glyph(latin, 0x00e9) ? |_| EvidenceFailure,
	)
	shape = KernelShape.validate_advanced_selected(
		multi_face_source,
		store,
		{
			faces: [
				{ font: latin, range: list_at(selection.face_ranges, 0) },
				{ font: cjk, range: list_at(selection.face_ranges, 1) },
				{ font: latin, range: list_at(selection.face_ranges, 2) },
			],
			occurrence: Semantics.OccurrenceId.from_index(0),
		},
		KernelShape.AdvancedLimits.make({ max_clusters: 3, max_glyph_indices: 3, max_glyphs: 3, max_runs: 3, max_scalars: 3, max_source_bytes: 6, max_substitutions: 0, max_transformations: 0 }),
	) ? |_| EvidenceFailure
	semantic = KernelTextSemantics.Plan.build(
	...
	scene = KernelScene.Plan.build(
		multi_face_scene,
		KernelScene.Resources.with_text({ color_spaces: 1, images: 0, text_runs: 3 }),
		KernelScene.Limits.make({ max_commands: 4, max_dash_lengths: 0, max_graphics_depth: 2, max_groups: 1, max_pages: 1, max_path_segments: 0, max_paths: 0 }),
	) ? |_| EvidenceFailure
	ownership = KernelTextOwnership.Plan.build(semantic, scene, shape.store) ? |_| EvidenceFailure
	latin_plan = KernelFontPlan.plan(latin, [{ glyph: list_at(shape.store.glyphs, 0).id.raw() }, { glyph: list_at(shape.store.glyphs, 2).id.raw() }], KernelFontPlan.Limits.make({ max_retained_glyphs: 8 })) ? |_| EvidenceFailure
	cjk_plan = KernelFontPlan.plan(cjk, [{ glyph: list_at(shape.store.glyphs, 1).id.raw() }], KernelFontPlan.Limits.make({ max_retained_glyphs: 8 })) ? |_| EvidenceFailure
	latin_subset = KernelFontSubset.build(latin, latin_plan) ? |_| EvidenceFailure
	cjk_subset = KernelFontSubset.build(cjk, cjk_plan) ? |_| EvidenceFailure
	colors = KernelColor.Plan.build(text_colors, KernelColor.Limits.make({ max_icc_bytes: 0, max_profiles: 0, max_spaces: 1, max_tags: 0 })) ? |_| EvidenceFailure
	images = KernelImage.Plan.build(empty_image_sources, colors, KernelImage.Limits.make({ max_decoded_bytes: 0, max_encoded_bytes: 0, max_height: 0, max_markers: 0, max_resources: 0, max_width: 0 })) ? |_| EvidenceFailure
	resource_use = KernelResourceUse.TextPlan.build(scene, colors, images) ? |_| EvidenceFailure
	text = KernelPdfText.ScenePlan.build(ownership, [latin_plan, cjk_plan], KernelPdfText.Limits.make({ max_actual_text_scalars: 3, max_content_bytes: 1024, max_mappings: 8, max_placements: 0, max_source_scalars: 3 })) ? |_| EvidenceFailure
	tagged = KernelTextOwnership.Plan.tagged(ownership)
	content = KernelContent.Plan.build_with_text(tagged, KernelPdfText.ScenePlan.content(text), KernelContent.Limits.make({ max_content_bytes: 2048, max_content_streams: 1 })) ? |_| EvidenceFailure
	base_objects = KernelObjectPlan.Plan.build_with_text(tagged, colors, images, resource_use, content, KernelObjectPlan.Limits.make({ max_objects: 48, max_pages: 1 })) ? |_| EvidenceFailure
	font_objects = KernelFontObjects.Plan.build(base_objects, 2, 48) ? |_| EvidenceFailure
	structure = KernelTaggedTextStructure.Plan.build(
		tagged,
		colors,
		images,
		...
		text,
		[
			{ descriptor, font: latin, plan: latin_plan, subset: latin_subset },
			{ descriptor, font: cjk, plan: cjk_plan, subset: cjk_subset },
		],
		KernelTaggedTextStructure.Limits.make({ font_limits: KernelPdfFont.Limits.make({ max_to_unicode_bytes: 2048, max_unicode_mappings: 8, max_unicode_scalars: 8 }), object_limits: multi_face_object_limits }),
	) ? |_| EvidenceFailure
	Ok({ cjk, cjk_plan, latin, latin_plan, selection, shape, structure, text })
}

multi_face_registry : {} -> Try(Font.Registry, [EvidenceFailure])
multi_face_registry = |_| {
	latin = Font.Registry.empty.register(caller_font_bytes, { provision: AdvancedRunsRequired, scripts: [Font.Script.from_iso15924("Latn")] }, Font.ValidationLimits.default) ? |_| EvidenceFailure
	cjk = latin.registry.register(cjk_font_bytes, { provision: AdvancedRunsRequired, scripts: [Font.Script.from_iso15924("Hani")] }, Font.ValidationLimits.default) ? |_| EvidenceFailure
	policy = cjk.registry.with_policy([latin.face, cjk.face]) ? |_| EvidenceFailure
	Ok(policy.registry)
}
...
build_sample_from_with_source_limits = |font, semantic, source_text, store, max_scalars, max_source_bytes, max_transformations, max_mappings, max_glyphs| {
	...
	base_objects = KernelObjectPlan.Plan.build_with_text(
		...
		resource_use,
		content,
		KernelObjectPlan.Limits.make({ max_objects: 32, max_pages: 1 }),
	) ? |_| ObjectFailure
	font_objects = KernelFontObjects.Plan.build(base_objects, 1, 32) ? |_| FontObjectFailure
	structure = KernelTaggedTextStructure.Plan.build(
		tagged,
...
## The three runs are exactly the three selected planner ranges. Their source
## ranges partition one Unicode source; the CJK run uses the CJK instance and
## the two Latin runs return to the caller instance without any fallback.
multi_face_store : List(Font.FaceRange), U32, U32, U32 -> Text.Store
multi_face_store = |ranges, latin_c, han, latin_eacute| {
	if ranges.len() != 3 {
		crash "validated multi-face selection escaped"
	}
	zero = Layout.Unit.from_raw(0)
	{
		clusters: [
			{ glyphs: Semantics.Range.from_start_and_length(0, 1), kind: OneToOne, source: multi_face_range(0) },
			{ glyphs: Semantics.Range.from_start_and_length(1, 1), kind: OneToOne, source: multi_face_range(1) },
			{ glyphs: Semantics.Range.from_start_and_length(2, 1), kind: OneToOne, source: multi_face_range(2) },
		],
		glyph_indices: [0, 1, 2],
		glyphs: [
			{ advance_x: Layout.Unit.from_raw(7000), advance_y: zero, id: Text.GlyphId.from_raw(latin_c), offset_x: zero, offset_y: zero },
			{ advance_x: Layout.Unit.from_raw(11000), advance_y: zero, id: Text.GlyphId.from_raw(han), offset_x: Layout.Unit.from_raw(7000), offset_y: zero },
			{ advance_x: Layout.Unit.from_raw(7000), advance_y: zero, id: Text.GlyphId.from_raw(latin_eacute), offset_x: Layout.Unit.from_raw(18000), offset_y: zero },
		],
		runs: [
			multi_face_run(0, list_at(ranges, 0), Font.Script.from_iso15924("Latn"), Language("en-AU"), multi_face_range(0)),
			multi_face_run(1, list_at(ranges, 1), Font.Script.from_iso15924("Hani"), Language("zh-Hans"), multi_face_range(1)),
			multi_face_run(2, list_at(ranges, 2), Font.Script.from_iso15924("Latn"), Language("en-AU"), multi_face_range(2)),
		],
		substitutions: [],
		transformations: [],
	}
...
multi_face_run : U64, Font.FaceRange, Font.Script, Semantics.Language, Semantics.TextRange -> Text.Run
multi_face_run = |index, range, script, language, run_source| {
	{
		actual_text: FromOccurrence,
		clusters: range.clusters,
		direction: LeftToRight,
		glyphs: Semantics.Range.from_start_and_length(index, 1),
		id: Text.RunId.from_index(index),
...
multi_face_source : Str
multi_face_source = "C中é"
...
multi_face_range : U64 -> Semantics.TextRange
multi_face_range = |index| match index {
	0 => { scalars: Semantics.Range.from_start_and_length(0, 1), utf8_bytes: Semantics.Range.from_start_and_length(0, 1) }
	1 => { scalars: Semantics.Range.from_start_and_length(1, 1), utf8_bytes: Semantics.Range.from_start_and_length(1, 3) }
	2 => { scalars: Semantics.Range.from_start_and_length(2, 1), utf8_bytes: Semantics.Range.from_start_and_length(4, 2) }
	_ => crash "validated multi-face source range escaped"
}
...
multi_face_semantics : Semantics.Store
multi_face_semantics = {
	..semantics,
	fragments: [{ ..list_at(semantics.fragments, 0), source_range: UnicodeRange({ scalars: Semantics.Range.from_start_and_length(0, 3), utf8_bytes: Semantics.Range.from_start_and_length(0, 6) }) }],
	occurrences: [{ ..list_at(semantics.occurrences, 0), source: Text(Semantics.TextSourceId.from_index(0), UnicodeRange({ scalars: Semantics.Range.from_start_and_length(0, 3), utf8_bytes: Semantics.Range.from_start_and_length(0, 6) })) }],
	text_sources: [{ unicode: multi_face_source }],
}

multi_face_scene : Scene.Store
multi_face_scene = {
	commands: [
		Transform({ children: Semantics.Range.from_start_and_length(1, 3), matrix: { a: Layout.Unit.from_raw(1000), b: Layout.Unit.from_raw(0), c: Layout.Unit.from_raw(0), d: Layout.Unit.from_raw(1000), e: Layout.Unit.from_raw(72000), f: Layout.Unit.from_raw(700000) } }),
		DrawText({ paint: { fill: { channels: Gray(0), space: Color.SpaceId.from_index(0) }, mode: Fill, opacity: 65535, stroke: NoStroke }, run: Text.RunId.from_index(0) }),
		DrawText({ paint: { fill: { channels: Gray(0), space: Color.SpaceId.from_index(0) }, mode: Fill, opacity: 65535, stroke: NoStroke }, run: Text.RunId.from_index(1) }),
		DrawText({ paint: { fill: { channels: Gray(0), space: Color.SpaceId.from_index(0) }, mode: Fill, opacity: 65535, stroke: NoStroke }, run: Text.RunId.from_index(2) }),
	],
	dash_lengths: [],
	groups: [{ commands: Semantics.Range.from_start_and_length(0, 1), id: Scene.GroupId.from_index(0), owner: Fragment(Semantics.FragmentId.from_index(0)) }],
	page_groups: [Scene.GroupId.from_index(0)],
	pages: [{ boxes: { art: a4_box, bleed: a4_box, crop: a4_box, media: a4_box, trim: a4_box }, id: Semantics.PageId.from_index(0), paint_order: Semantics.Range.from_start_and_length(0, 1), rotation: Rotate0 }],
	path_segments: [],
	paths: [],
}
...
list_set = |items, index, value| match items.set(index, value) {
	Ok(updated) => updated
	Err(OutOfBounds) => {
		crash "text-layout actual-text evidence write escaped"
	}
}
...
list_at = |items, index| match items.get(index) {
	Err(OutOfBounds) => {
		crash "text-layout ActualText evidence index escaped"
	}
	Ok(value) => value
}
