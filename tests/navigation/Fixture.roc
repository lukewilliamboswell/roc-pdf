...
## production-visual navigation and link-annotation evidence.
##
...
## page-label ranges), runs the whole canonical production-visual pipeline with
...
## The canonical production-visual pipeline with navigation: the navigation store
...
run_pipeline = |input, guard| {
	...
	base = KernelObjectPlan.Plan.build_canonical(
		...
		KernelObjectPlan.Limits.make({ max_objects: 8192, max_pages: 128 }),
	) ? ObjectFailure
	...
	structure = KernelFormStructure.Plan.build_with_navigation(
		tagged,
		colors,
		images,
		content,
...
check_invariants : Built -> Try({}, Fixture.EvidenceError)
check_invariants = |built| {
	structure_work = KernelFormStructure.Plan.work(built.structure)
...
expect_rejection : U64, KernelNavigation.Input, KernelNavigation.Context, KernelNavigation.Limits, (Document.NavigationError -> Bool) -> Try({}, Fixture.EvidenceError)
