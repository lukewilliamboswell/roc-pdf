...
## production-visual transparency evidence.
##
...
expect_facts_rejection : U64, Scenario, KernelForm.Limits, (KernelForm.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
expect_plan_rejection : U64, Scenario, KernelForm.Limits, (BuildFailure -> Bool) -> Try({}, Fixture.EvidenceError)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "production-visual transparency evidence index escaped"
	}
}
