...
main! = |args| {
	context = match args.get(1) {
		Ok(text) => match U64.from_str(text) {
			Ok(value) => value
			Err(_) => {
				crash "Gate 4 navigation facade negative context must be an unsigned integer"
			}
		}
		Err(OutOfBounds) => {
			crash "Gate 4 navigation facade negatives require a runtime context"
		}
	}
	guard = U64.mod_by(context, 1)
	passed = run_negatives(guard)
	if passed != 9 {
		crash "Gate 4 navigation facade negatives failed: ${passed.to_str()} of 9"
	}
	...
	bytes = match Pdf.to_bytes(carrier) {
		Ok(value) => value
		Err(_) => {
			crash "Gate 4 navigation facade carrier failed"
		}
	}
