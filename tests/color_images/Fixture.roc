...
## production-visual color and image leaf evidence.
##
...
run_pipeline = |input| {
	...
	structure = KernelFormStructure.Plan.build(tagged, colors, images, content, form_plan, objects, NoTextObjects, structure_limits) ? StructureFailure
...
expect_color_rejection : U64, Color.Store, KernelColor.Limits, (KernelColor.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
expect_image_rejection : U64, Image.SourceStore, KernelImage.Limits, (KernelImage.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "production-visual color-image evidence index escaped"
	}
}
...
list_set = |items, index, value| match items.set(index, value) {
	Ok(next) => next
	Err(OutOfBounds) => {
		crash "production-visual color-image evidence update escaped"
	}
}
