...
## production-visual metadata, document-language, and output-intent evidence.
##
## Every positive scenario validates authored metadata facts once, serializes
## the canonical XMP packet once, runs the whole canonical production-visual pipeline
...
## The canonical production-visual pipeline with document facts: metadata validation and
...
run_pipeline = |input| {
	...
	base = KernelObjectPlan.Plan.build_canonical(
		tagged,
		colors,
		images,
		resource_use,
		...
		KernelObjectPlan.Limits.make({ max_objects: 8192, max_pages: 1 }),
	) ? ObjectFailure
	...
	structure = KernelFormStructure.Plan.build_with_facts(
		tagged,
		colors,
		images,
		content,
...
check_sharing : Built, U64, U64 -> Try({}, Fixture.EvidenceError)
check_sharing = |built, authored, canonical| {
	...
	plan = KernelFormStructure.Plan.structure(built.structure)
...
expect_metadata_rejection : U64, { created : Metadata.TimestampInput, language : Str, modified : Metadata.TimestampInput, title : Str }, (Metadata.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
expect_intent_rejection : U64, Color.OutputIntent, Str, Color.Store, (KernelMetadata.IntentError -> Bool) -> Try({}, Fixture.EvidenceError)
...
expect match Fixture.scenario("share", 5) {
	Ok(result) => list_at(result.work, 0) == 5 and list_at(result.work, 1) == 1
	Err(_) => False
}

expect match Fixture.scenario("minimal", 0) {
	Ok(result) => list_at(result.work, 10) == 2
	Err(_) => False
}

expect match Fixture.scenario("title", 8) {
	Ok(result) => list_at(result.work, 11) == 8
	Err(_) => False
}

expect match Fixture.atomic_negatives(1) {
