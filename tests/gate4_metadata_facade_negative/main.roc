...
main! = |args| {
	context = match args.get(1) {
		Ok(text) => match U64.from_str(text) {
			Ok(value) => value
			Err(_) => {
				crash "Gate 4 facade metadata negative context must be an unsigned integer"
			}
		}
		Err(OutOfBounds) => {
			crash "Gate 4 facade metadata negatives require a runtime context"
		}
	}
	if context != 1 {
		crash "Gate 4 facade metadata negatives expect context 1"
	}
	...
	carrier = match Pdf.to_bytes(make("en-AU", valid_title)) {
		Ok(value) => value
		Err(_) => {
			crash "Gate 4 facade metadata carrier generation failed"
		}
	}
