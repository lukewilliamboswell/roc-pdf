...
## production-visual shading and tiling-pattern evidence.
##
...
run_pipeline = |input| {
	...
	objects = KernelFormObjects.Plan.build_with_paints(
		base,
	...
	structure = KernelFormStructure.Plan.build_with_paints(tagged, colors, images, content, form_plan, objects, NoTextObjects, input.shadings, structure_limits) ? StructureFailure
...
expect_paint_rejection : U64, Scenario, (KernelScene.Error -> Bool) -> Try({}, Fixture.EvidenceError)
...
list_at = |items, index| match items.get(index) {
	Ok(value) => value
	Err(OutOfBounds) => {
		crash "production-visual shading-pattern evidence index escaped"
	}
}
