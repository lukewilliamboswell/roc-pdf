...
## The public one-import facade path for the Gate 4 metadata slice: a
...
## bytes are checked structurally by scripts/check_gate4_metadata.py.
facade_title : Str
...
main! = |args| {
	mode = match args.get(1) {
		Ok(text) => text
		Err(OutOfBounds) => {
			crash "Gate 4 facade metadata evidence requires a mode"
		}
	}
	...
	bytes = match Pdf.to_bytes(document) {
		Ok(value) => value
		Err(_) => {
			crash "Gate 4 facade metadata generation failed"
		}
	}
	if mode == "twice" {
		second = match Pdf.to_bytes(facade_document(title)) {
			Ok(value) => value
			Err(_) => {
				crash "Gate 4 facade metadata regeneration failed"
			}
		}
